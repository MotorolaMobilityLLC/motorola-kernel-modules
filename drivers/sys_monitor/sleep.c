...
static int *pm_wakeup_irq_ptr = NULL;

static unsigned long lookup_kallsyms_addr(const char *name)
{
    struct file *f;
    char buf[512];
    loff_t pos = 0;
    ...
    while ((ret = kernel_read(f, buf, sizeof(buf) - 1, &pos)) > 0) {
        char *p, *s;
        buf[ret] = '\0';
        p = strchr(buf, '\n');
        if (p)
...
if (pm_wakeup_irq_ptr)
    suspend_state[cur_idx].wakeup_irq = *pm_wakeup_irq_ptr;
else
    suspend_state[cur_idx].wakeup_irq = 0;
...
int monitor_sleep_init(void)
{
    ...
    addr = lookup_kallsyms_addr("pm_wakeup_irq");
    if (addr) {
        pm_wakeup_irq_ptr = (int *)addr;
        pr_info("Found pm_wakeup_irq at 0x%lx\n", addr);
    } else {
        pr_err("Failed to find pm_wakeup_irq symbol\n");
    }
