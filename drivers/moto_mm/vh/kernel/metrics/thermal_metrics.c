...
static struct kobject moto_metrics_kobj;
#endif
...
#else
	moto_metrics_kobj.kset = module_kset;
	err = kobject_init_and_add(&moto_metrics_kobj, &module_ktype, NULL,
				   "moto_metrics");
	if (err) {
		pr_err("Failed to init moto_metrics_kobj\n");
		return err;
	}
	mod_uevent_kobj = &moto_metrics_kobj;
#endif
	designated_handle = -1;
	if (!metrics_kobj) {
