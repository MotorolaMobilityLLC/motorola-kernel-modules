...
#define CONFIG_VH_SCHED_MAX_CPU_NR 8
struct long_irq {
...
void vh_sched_switch_moto_mod(void *data, bool preempt,
...
static ssize_t runnable_stats_enable_store(struct kobject *kobj,
					  ...
					  size_t count)
{
	register_trace_sched_wakeup(vh_sched_wakeup_moto_mod, NULL);
	register_trace_sched_switch(vh_sched_switch_moto_mod, NULL);
	return count;
}
...
static ssize_t runnable_stats_disable_store(struct kobject *kobj,
					  ...
					  size_t count)
{
	unregister_trace_sched_wakeup(vh_sched_wakeup_moto_mod, NULL);
	unregister_trace_sched_switch(vh_sched_switch_moto_mod, NULL);
	return count;
}
...
int perf_metrics_init(struct kobject *metrics_kobj)
{
	...
	ret = register_trace_sched_wakeup(vh_sched_wakeup_moto_mod, NULL);
	if (ret)
		return ret;

	ret = register_trace_sched_switch(vh_sched_switch_moto_mod, NULL);
	if (ret)
		return ret;
