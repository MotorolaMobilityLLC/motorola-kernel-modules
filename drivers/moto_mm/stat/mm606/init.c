...
static int moto_stat_mm_init(void)
{
	...
	ret = register_trace_android_vh_alloc_pages_slowpath_start(vh_slowpath_begin, NULL);
	if (ret)
		return ret;
	ret = register_trace_android_vh_alloc_pages_slowpath_end(vh_slowpath_end, NULL);
	if (ret)
		return ret;
