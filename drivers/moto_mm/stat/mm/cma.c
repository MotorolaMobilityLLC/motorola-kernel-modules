...
static struct cma_moto_stat *stats[MAX_CMA_AREAS];
...
{
	struct cma_moto_stat *cma_stat =
		container_of(kobj, struct cma_moto_stat, kobj);
...
{
	kfree(container_of(kobj, struct cma_moto_stat, kobj));
}
...
static struct kobject *moto_cma_kobj;
...
int create_cma_sysfs(struct kobject *mm_kobj)
{
	...
	moto_cma_kobj = kobject_create_and_add("cma", mm_kobj);
	if (!moto_cma_kobj)
		return -ENOMEM;
