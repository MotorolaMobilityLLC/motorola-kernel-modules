...
struct direct_reclaim_moto_stat {
	spinlock_t lock;
...
static struct direct_reclaim_moto_stat *stats[OOM_SCORE_ADJ_NUM_LEVELS];
...
{
	kfree(container_of(kobj, struct direct_reclaim_moto_stat, kobj));
}
...
static struct kobject *moto_vmscan_kobj;
static struct kobject *moto_direct_reclaim_kobj;
...
void remove_vmscan_sysfs(void)
{
	...
	kobject_put(moto_direct_reclaim_kobj);
	kobject_put(moto_vmscan_kobj);
}
...
static int add_direct_reclaim_sysfs(struct kobject *direct_reclaim_kobj)
{
	...
	for (i = 0; i < OOM_SCORE_ADJ_NUM_LEVELS; i++) {
		stats[i] = kzalloc(sizeof(struct direct_reclaim_moto_stat),
				GFP_KERNEL);
...
int create_vmscan_sysfs(struct kobject *mm_kobj)
{
	...
	moto_vmscan_kobj = kobject_create_and_add("vmscan", mm_kobj);
	if (!moto_vmscan_kobj) {
		retval = -ENOMEM;
		goto out;
	}

	moto_direct_reclaim_kobj = kobject_create_and_add("direct_reclaim",
			moto_vmscan_kobj);
	if (!moto_direct_reclaim_kobj) {
		retval = -ENOMEM;
		goto err_direct_out;
	}

	retval = add_direct_reclaim_sysfs(moto_direct_reclaim_kobj);
