...
static struct class *fts_touchscreens_class;
...
static ssize_t fts_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    char panel_name[FILE_NAME_LENGTH] = { 0 };
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    struct input_dev *input_dev = ts_data->input_dev;
    ...
    strncpy(panel_name, buf, copy_count);
    panel_name[copy_count] = '\0';
    FTS_TEST_DBG("panel_name:%s", panel_name);

    memset(fwname, 0, sizeof(fwname));
    snprintf(fwname, FILE_NAME_LENGTH, "%s%s", panel_name, FTS_LIMIT_FILE_NAME);
...
int fts_test_init(struct fts_ts_data *ts_data)
{
    ...
    fts_touchscreens_class = class_create("fts_touchscreen");
#else
    fts_touchscreens_class = class_create(THIS_MODULE, "fts_touchscreen");
#endif
    if (IS_ERR(fts_touchscreens_class)) {
        ret = PTR_ERR(fts_touchscreens_class);
        fts_touchscreens_class = NULL;
        FTS_ERROR("fts_touchscreen_class: touchscreen class init fail!");
        return ret;
    }

    ret = alloc_chrdev_region(&ts_data->class_dev_no, 0, 1, "fts_test");
    if (ret < 0) {
        FTS_ERROR("get device number failed %d", ret);
        return ret;
    }

    ts_data->class_dev = device_create(fts_touchscreens_class,
        NULL, ts_data->class_dev_no,
        ts_data, "%s", "fts_test");
    if (IS_ERR(ts_data->class_dev)) {
        ret = PTR_ERR(ts_data->class_dev);
        ts_data->class_dev = NULL;
        unregister_chrdev_region(ts_data->class_dev_no, 1);
        return ret;
    }

    ret = sysfs_create_group(&ts_data->class_dev->kobj, &fts_test_attribute_group);
    if (0 != ret) {
        FTS_TEST_ERROR("sysfs(test) create fail");
        sysfs_remove_group(&ts_data->class_dev->kobj, &fts_test_attribute_group);
        device_unregister(ts_data->class_dev);
        unregister_chrdev_region(ts_data->class_dev_no, 1);
        return ret;
    } else {
...
int fts_test_exit(struct fts_ts_data *ts_data)
{
    ...
    device_unregister(ts_data->class_dev);
    unregister_chrdev_region(ts_data->class_dev_no, 1);
    if (fts_touchscreens_class)
        class_destroy(fts_touchscreens_class);
    if (tdata) {
#if CSV_SUPPORT
