DLKM_DIR := motorola/kernel/modules
LOCAL_PATH := $(call my-dir)

#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,15,0)
ifeq ($(DLKM_INSTALL_TO_VENDOR_OUT),true)
ILITEK_MMI_MODULE_PATH := $(TARGET_OUT_VENDOR)/lib/modules/
else
ILITEK_MMI_MODULE_PATH := $(KERNEL_MODULES_OUT)
endif
#endif

ifneq ($(BOARD_USES_DOUBLE_TAP),)
	KERNEL_CFLAGS += CONFIG_INPUT_ILI_ENABLE_DOUBLE_TAP=y
	KBUILD_OPTIONS += CONFIG_INPUT_ILI_ENABLE_DOUBLE_TAP=y
endif

ifeq ($(BOARD_USES_DOUBLE_TAP_CTRL),true)
	KERNEL_CFLAGS += CONFIG_BOARD_USES_DOUBLE_TAP_CTRL=y
	KERNEL_OPTIONS += CONFIG_BOARD_USES_DOUBLE_TAP_CTRL=y
endif

ifneq ($(MOTO_PANEL_CHECK_TOUCH_STATE),)
	KERNEL_CFLAGS += CONFIG_INPUT_ILI_SET_TOUCH_STATE=y
	KBUILD_OPTIONS += CONFIG_INPUT_ILI_SET_TOUCH_STATE=y
endif

ifneq ($(BOARD_USES_PANEL_NOTIFICATIONS),)
	KERNEL_CFLAGS += CONFIG_INPUT_ILI_PANEL_NOTIFICATIONS=y
endif

ifneq ($(BOARD_USES_MTK_CHECK_PANEL),)
	KERNEL_CFLAGS += CONFIG_ILITEK_MTK_CHECK_PANEL=y
endif

ifeq ($(ILITEK_MULTI_TP_MODULE),true)
	KERNEL_CFLAGS += CONFIG_ILITEK_MULTI_TP_MODULE=y
endif

ifeq ($(CONFIG_STOWED_MODE_EN),true)
	KERNEL_CFLAGS += CONFIG_STOWED_MODE_EN=y
	KBUILD_OPTIONS += CONFIG_STOWED_MODE_EN=y
endif

ifneq ($(ILITEK_RESUME_BY_DDI),)
	KERNEL_CFLAGS += CONFIG_ILITEK_RESUME_BY_DDI=y
endif

ifeq ($(CONFIG_INPUT_CHARGER_DETECTION),true)
	KERNEL_CFLAGS += CONFIG_ILITEK_CHARGER=y
endif

ifeq ($(ILITEK_ESD),true)
	KERNEL_CFLAGS += CONFIG_ILITEK_ESD=y
endif

ifeq ($(ILITEK_GESTURE),true)
	KERNEL_CFLAGS += CONFIG_ILITEK_GESTURE=y
endif

ifeq ($(ILITEK_DEBUG_INFO),true)
	KERNEL_CFLAGS += CONFIG_ILI_DEBUG_INFO=y
	KBUILD_OPTIONS += CONFIG_ILI_DEBUG_INFO=y
endif

ifeq ($(TOUCHSCREEN_LAST_TIME),true)
	KERNEL_CFLAGS += CONFIG_ILI_TOUCH_LAST_TIME=y
	KERNEL_OPTIONS += CONFIG_ILI_TOUCH_LAST_TIME=y
endif

ifneq ($(findstring mmi_info.ko,$(BOARD_VENDOR_KERNEL_MODULES)),)
      KERNEL_CFLAGS += CONFIG_INPUT_TOUCHSCREEN_MMI=y
      KBUILD_OPTIONS += CONFIG_INPUT_TOUCHSCREEN_MMI=y
 endif

ifeq ($(CONFIG_ILITEK_LOG_CAPTURE),true)
	KERNEL_CFLAGS += CONFIG_ENABLE_TP_ILI_LOG_CAPTURE=y
	KBUILD_OPTIONS += CONFIG_ENABLE_TP_ILI_LOG_CAPTURE=y
endif

ifeq ($(CONFIG_INPUT_CHECK_DEVICE_BOOTMODE),true)
	KERNEL_CFLAGS += CONFIG_ILITEK_CHECK_DEVICE_BOOTMODE=y
	KBUILD_OPTIONS += CONFIG_ILITEK_CHECK_DEVICE_BOOTMODE=y
endif

include $(CLEAR_VARS)
ifneq ($(BOARD_USES_DOUBLE_TAP),)
LOCAL_ADDITIONAL_DEPENDENCIES += $(KERNEL_MODULES_OUT)/sensors_class.ko
LOCAL_REQUIRED_MODULES := sensors_class.ko
endif
LOCAL_MODULE := ilitek_v4_mmi.ko
LOCAL_MODULE_TAGS := optional
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,15,0)
LOCAL_MODULE_PATH := $(ILITEK_MMI_MODULE_PATH)
#else
LOCAL_MODULE_PATH := $(KERNEL_MODULES_OUT)
#endif
KBUILD_OPTIONS_GKI += GKI_OBJ_MODULE_DIR=gki
LOCAL_ADDITIONAL_DEPENDENCIES += $(KERNEL_MODULES_OUT)/mmi_info.ko
ifneq ($(findstring touchscreen_mmi.ko,$(BOARD_VENDOR_KERNEL_MODULES)),)
	LOCAL_ADDITIONAL_DEPENDENCIES += $(KERNEL_MODULES_OUT)/touchscreen_mmi.ko
endif
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,15,0)
ifneq ($(MOTO_PANEL_CHECK_TOUCH_STATE_V2),)
	LOCAL_ADDITIONAL_DEPENDENCIES += $(KERNEL_MODULES_OUT)/msm_drm.ko
endif
KBUILD_OPTIONS += MODULE_KERNEL_VERSION=$(TARGET_KERNEL_VERSION)
KBUILD_OPTIONS_GKI += GKI_OBJ_MODULE_DIR=gki
#endif
include $(DLKM_DIR)/AndroidKernelModule.mk
