...
#if AXIS_PACKET
	ilits->tp_data_len += P5_X_DEMO_MODE_PACKET_INFO_LEN + P5_X_DEMO_MODE_AXIS_LEN + P5_X_DEMO_MODE_STATE_INFO;
#endif
...
int ili_interface_dev_init(struct ilitek_hwif_info *hwif)
{
	...
	info->hwif = hwif;
    ILI_INFO("match table: %s\n", hwif->of_match_table->compatible);
