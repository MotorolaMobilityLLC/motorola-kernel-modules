...
int ili_check_dt() {
	int ret = 0;
        ...
        if (node1) {
                int ret;
		ret = of_property_read_string(node1, "ili,panel-supplier", &tpd_device_driver.tpd_panel_supplier);
		ILI_INFO("get ili,panel_supplier ret=%d, tpd_panel_supplier= %s\n", ret, tpd_device_driver.tpd_panel_supplier);
        } else {
                ILI_INFO("node not found!\n");
        }
...
static int __init ilitek_plat_dev_init(void)
{
	...
	if (ret < 0) {
		...
		//Neen't remove when add fail. fix NULL panic for other multiple panels of same product
