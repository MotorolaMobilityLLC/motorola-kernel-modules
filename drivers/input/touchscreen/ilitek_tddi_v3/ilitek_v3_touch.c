...
static void dma_clear_reg_setting(void)
{
	...
	if (ili_ice_mode_bit_mask_write(INTR32_ADDR, INTR32_reg_t0_int_en | INTR32_reg_t1_int_en, 0) < 0)
		ILI_ERR("Mask write: mask=%lu, value=%d, addr=0x%x failed\n", INTR32_reg_t0_int_en | INTR32_reg_t1_int_en, 0, INTR32_ADDR);
	...
	if (ili_ice_mode_bit_mask_write(DMA48_ADDR, DMA48_reg_dma_ch0_trigger_sel | BIT(20), 0) < 0)
		ILI_ERR("Mask write: mask=%lu, value=%d, addr=0x%x failed\n", DMA48_reg_dma_ch0_trigger_sel | BIT(20), 0, DMA48_ADDR);
...
{
	/* 1. interrupt t0/t1 enable flag */
	if (ili_ice_mode_bit_mask_write_cascade(INTR32_ADDR, INTR32_reg_t0_int_en | INTR32_reg_t1_int_en, 0, BOTH) < 0)
	...
	/* 4. clear dma channel 0 trigger select */
	if (ili_ice_mode_bit_mask_write_cascade(DMA48_ADDR, DMA48_reg_dma_ch0_trigger_sel | BIT(20), 0, BOTH) < 0)
		ILI_ERR("Cascade mask write: mask=%lu, value=%d, addr=0x%x failed\n", DMA48_reg_dma_ch0_trigger_sel | BIT(20), 0, DMA48_ADDR);
...
static void dma_trigger_reg_setting(u32 reg_dest_addr, u32 flash_start_addr, u32 copy_size)
{
	...
	if (ili_ice_mode_bit_mask_write(DMA52_ADDR, DMA52_reg_dma_ch0_src2_en, 0) < 0)
		ILI_ERR("Mask write: mask=%lu, value=%d, addr=0x%x failed\n", DMA52_reg_dma_ch0_src2_en, 0, DMA52_ADDR);
	...
	if (ili_ice_mode_bit_mask_write(DMA55_ADDR, DMA55_reg_dma_ch0_trafer_mode, 0) < 0)
		ILI_ERR("Mask write: mask=%lu, value=%d, addr=0x%x failed\n", DMA55_reg_dma_ch0_trafer_mode, 0, DMA55_ADDR);
...
static void dma_trigger_reg_setting_cascade(u32 reg_dest_addr, u32 flash_start_addr, u32 copy_size)
{
	...
	/* 3. set dma channel 0 src2 info */
	if (ili_ice_mode_bit_mask_write_cascade(DMA52_ADDR, DMA52_reg_dma_ch0_src2_en, 0, BOTH) < 0)
		ILI_ERR("Cascade mask write: mask=%lu, value=%d, addr=0x%x failed\n", DMA52_reg_dma_ch0_src2_en, 0, DMA52_ADDR);
	...
	if (ili_ice_mode_write_by_mode(DMA55_reg_dma_ch0_trafer_counts, copy_size, 4, BOTH) < 0)
		ILI_ERR("Cascade write %x at %x failed\n", copy_size, DMA55_reg_dma_ch0_trafer_counts);
	if (ili_ice_mode_bit_mask_write_cascade(DMA55_ADDR, DMA55_reg_dma_ch0_trafer_mode, 0, BOTH) < 0)
		ILI_ERR("Cascade mask write: mask=%lu, value=%d, addr=0x%x failed\n", DMA55_reg_dma_ch0_trafer_mode, 0, DMA55_ADDR);
...
int ili_move_gesture_code_iram(int mode)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		if (ret < 0) {
		ILI_ERR("wrapper failed when cmd write\n");
		goto out;
		}
	...
	if (ilits->prox_face_mode == PROXIMITY_SUSPEND_RESUME && ilits->proxmity_face == true) {
		ret = ili_ic_func_ctrl("proximity", 0x01); // Call function but don't assign to ret
		if (ret < 0) {
			ILI_ERR("Write func proximity ctrl cmd failed\n");
		}
		ret = ili_ic_func_ctrl("lpwg", 0x21);
		if (ret < 0) {
			ILI_ERR("Write func lpwg ctrl cmd failed\n");
		}
	}
...
u8 ili_calc_packet_checksum(const u8 *packet, int len)
{
	int i;
	s32 sum = 0;
...
static int ili_i2c_write_ges_password(int pwd_adr, int pwd, int pwd_len)
{
	int ret = 0;
	u32 answer = 0;
	...
	if (ilits->cascade_info_block.nNum != 0) {
		ret = ili_ice_mode_write_by_mode(pwd_adr, pwd, pwd_len, BOTH);
	} else {
		ret = ili_ice_mode_write(pwd_adr, pwd, pwd_len);
	}
	...
	/* Read password address for Confirm password is been write correct*/
	/*Slave*/
	if (ilits->cascade_info_block.nNum != 0) {
		ret = ili_ice_mode_read_by_mode(pwd_adr, &answer, pwd_len, SLAVE);
		...
		if (answer != pwd) {
			ILI_INFO("Slave ret = 0x%X, pwd = 0x%X\n", answer, pwd);
			return -1;
		}
	}
	...
	/*Master*/
	answer = 0;
	ret = ili_ice_mode_read_by_mode(pwd_adr, &answer, pwd_len, MASTER);
	...
	if (answer != pwd) {
		ILI_INFO("Master ret = 0x%X, pwd = 0x%X\n", answer, pwd);
		return -1;
	}
...
{
	int ret, try_cnt = 0; 
	int retry_times_pwd = 4;/* number of retry to confirm password have been write succesfully */
	int retry_times_ges_run = 100;/* number of retry to confirm gesture run key is correct value */
	...
	do {
		try_cnt ++;
		ILI_INFO("Write ges password %d's\n", try_cnt);

		if (ili_i2c_write_ges_password(ges_pwd_addr, ges_pwd, pwd_len) < 0) {
			/*reset when write fail*/
			if (ilits->cascade_info_block.nNum != 0)
	...
	} while (try_cnt < retry_times_pwd);

	if (try_cnt >= retry_times_pwd) {
		ILI_ERR("Write gesture password fail\n");
		ret = -1;
		goto out;
	}
	...
	if (ilits->cascade_info_block.nNum != 0) {
		...
		/*mdelay(85);*/ /*Not WDT Reset don't need addtional delay*/
	} else {
	...
	if (ilits->cascade_info_block.nNum != 0) {
		do {
			...
			if (slave_answer != ges_run) {
				ILI_INFO("Try %d's : slave ret = 0x%X, ges_run = 0x%X\n", try_cnt, slave_answer, ges_run);
				atomic_set(&ilits->ice_stat, DISABLE);/*force enter ice again*/
			...
			mdelay(2);
		} while (loop_flag && try_cnt < retry_times_ges_run);

		if (try_cnt >= retry_times_ges_run) {
	...
	do {
		...
		if (answer != ges_run) {
			ILI_INFO("Try %d's : master ret = 0x%X, ges_run = 0x%X\n", try_cnt, answer, ges_run);
			atomic_set(&ilits->ice_stat, DISABLE);/*force enter ice again*/
			ilits->ice_mode_ctrl(ENABLE, ON, BOTH);
			loop_flag = true;
		} else 
...
out:
	if (ilits->ice_mode_ctrl(DISABLE, ON, BOTH)) {
...
static int ili_spi_write_ges_password(int pwd_adr, int pwd, int pwd_len)
{
	u32 answer = 0;
	...
	/* write a special password to inform FW go back into gesture mode */
	if (ili_ice_mode_write(pwd_adr, pwd, pwd_len) < 0) {
		ILI_ERR("write password failed\n");
		return -1;
	}
	...
	/*Slave*/
	if (ilits->cascade_info_block.nNum != 0) {
		ili_spi_ice_mode_read(pwd_adr, &answer, pwd_len, SLAVE);
	...
	/*Master*/
	answer = 0;
	ili_spi_ice_mode_read(pwd_adr, &answer, pwd_len, MASTER);
...
int ili_touch_esd_gesture_iram(void)
{
	...
	int try_cnt = 0;
	int retry_times_pwd = 4;/* number of retry to confirm password have been write succesfully */
	int retry_times_gesrun = 100;/* number of retry to confirm password have been write succesfully */
	bool loop_flag = false;
	int pwd_len = 2;
	u8 data_type = P5_X_FW_SIGNAL_DATA_MODE;
	u8 backup_rec_state = 0;
	...
	do {
		...
		if (ili_spi_write_ges_password(ges_pwd_addr, ges_pwd, pwd_len) < 0) {
			/*reset when write fail*/
			if (ilits->cascade_info_block.nNum != 0) {
	...
	if (ilits->cascade_info_block.nNum != 0) {
		...
		do {
			try_cnt ++;
			ili_spi_ice_mode_read(ges_pwd_addr, &answer, pwd_len, SLAVE);

			if (answer != ges_run) {
				ILI_INFO("Try %d's : ret = 0x%X, slave answer = 0x%X\n",try_cnt , answer, ges_run);
				atomic_set(&ilits->ice_stat, DISABLE);/*force enter ice again*/
				ilits->ice_mode_ctrl(ENABLE, ON, BOTH);
				loop_flag = true;
			} else
			...
			mdelay(2);
		} while (loop_flag && try_cnt < retry_times_gesrun);
	}
	if (try_cnt >= retry_times_gesrun) {
		ILI_ERR("Enter gesture failed\n");
		ret = -EFW_GESRUN_FAIL;
		goto fail;
	}
	...
	do {
		...
		ili_spi_ice_mode_read(ges_pwd_addr, &answer, pwd_len, MASTER);

		if (answer != ges_run) {
			ILI_INFO("Try %d's : ret = 0x%X, answer = 0x%X\n", try_cnt, answer, ges_run);
			atomic_set(&ilits->ice_stat, DISABLE);/*force enter ice again*/
			ilits->ice_mode_ctrl(ENABLE, ON, BOTH);
			loop_flag = true;
		} else
	...
	if (ilits->prox_face_mode == PROXIMITY_SUSPEND_RESUME) {
		...
		ret = ili_ic_func_ctrl("lpwg", 0x6);
		if (ret < 0) {
			ILI_ERR("ili_ic_func_ctrl error");
			goto fail;
		}
		...
		if (ilits->proxmity_face == true) {
			ret = ili_ic_func_ctrl("proximity", 0x01);
			if (ret < 0) {
				ILI_ERR("ili_ic_func_ctrl proximity error");
				goto fail;
			}
			ret = ili_ic_func_ctrl("lpwg", 0x21);
			if (ret < 0) {
				ILI_ERR("ili_ic_func_ctrl lpwg error");
				goto fail;
			}
		}
	} else {
...
void ili_pen_debug_mode_report_point(u8 *buf, int len, u8 offset)
{
	...
	for (i = 0; i < ilits->touch_num; i++) {
		...
		/* cdc debug lite */
		if (ilits->tp_data_format == DATA_FORMAT_DEBUG_LITE_ROI
			|| ilits->tp_data_format == DATA_FORMAT_DEBUG_LITE_WINDOW
			|| ilits->tp_data_format == DATA_FORMAT_DEBUG_LITE_AREA
			|| ilits->tp_data_format == DATA_FORMAT_DEBUG_LITE_ALS) {
...
static void ili_debug_mode_report_point(const u8 *buf, int len)
{
	int i = 0;
	u32 xop = 0, yop = 0;
	static u8 p[MAX_TOUCH_NUM];
