...
static struct mp_test_items tItems[MP_TEST_ITEM] = {
	...
	{.desp = "noise peak to peak(with panel) i", .catalog = PEAK_TO_PEAK_TEST_I, .lcm = ON},
	...
	{.desp = "freqpoint_noise_p2p_1", .catalog = PEAK_TO_PEAK_TEST, .lcm = ON},
	{.desp = "freqpoint_noise_p2p_2", .catalog = PEAK_TO_PEAK_TEST, .lcm = ON},
	/* LCM OFF TEST */
	...
	{.desp = "noise peak to peak(with panel) i (lcm off)", .catalog = PEAK_TO_PEAK_TEST_I, .lcm = OFF},
...
static int parser_get_u32_array(char *key, u32 *buf, u16 base, int len, const char *delimiter)
{
	char *s = key;
	char *pToken;
	int ret, conut = 0;
	 ...
	 */
	if (isspace_t((int)(unsigned char)*s) == 0) {
		while ((pToken = strsep(&s, delimiter)) != NULL) {
			ret = kstrtol(pToken, base, &s_to_long);
			if (ret == 0)
				buf[conut] = s_to_long;
			else
	...
	while (true) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				ini_buf[n - 1] = 0x00;
				strncpy((char *)tmpSectionName, ini_buf + 1, (PARSER_MAX_CFG_BUF) * sizeof(char));
				tmpSectionName[PARSER_MAX_CFG_BUF - 1] = '\0';
		...
		strncpy(ini_info[g_ini_items].section_name, tmpSectionName, sizeof(ini_info[g_ini_items].section_name) - 1);
		ini_info[g_ini_items].section_name[sizeof(ini_info[g_ini_items].section_name) - 1] = '\0';
		...
		if (isEqualSign == 0) {
			...
			if (strstr(ini_info[g_ini_items].section_name, BENCHMARK_KEY_NAME) != NULL) {
				...
				isEqualSign = -1;
				ini_info[g_ini_items].key_name_len = strlen(ini_info[g_ini_items].section_name);
				strncpy(ini_info[g_ini_items].key_name, ini_info[g_ini_items].section_name, (PARSER_MAX_KEY_NAME_LEN * sizeof(char))- 1);
				ini_info[g_ini_items].key_name[(PARSER_MAX_KEY_NAME_LEN * sizeof(char))- 1] = '\0';
...
{
	int i, x, y, k, ret = 0;
	const int border_x[] = {-1, 0, 1, 1, 1, 0, -1, -1};
	const int border_y[] = {-1, -1, -1, 0, 1, 1, 1, 0};
	s32 *p_comb = frame_buf;
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line, int file_size)
{
	...
	tmp_len += snprintf(csv + tmp_len, (file_size - tmp_len), "==============================================================================\n");
	tmp_line++;
	tmp_len += snprintf(csv + tmp_len, (file_size - tmp_len), "Firmware AP Version ,0x%x\n", core_mp.fw_ver);
	tmp_line++;
...
static void mp_compare_cdc_show_result(int index, s32 *tmp, char *csv,
				int *csv_len, int type, const s32 *max_ts,
				const s32 *min_ts, const char *desp, int file_zise)
{
...
static void mp_compare_tip_ring_cdc_show_result(int index, s32 *tmp, char *csv,
				int *csv_len, int type, const s32 *max_ts, const s32 *min_ts, const char *desp,
...
static int compare_charge(const s32 *charge_rate, int x, int y, const s32 *inNodeType,
		int Charge_AA, int Charge_Border, int Charge_Notch)
{
	int OpenThreadhold, tempY, tempX, ret, k;
	const int sx[] = {-1, 0, 1, -1, 1, -1, 0, 1};
	const int sy[] = {-1, -1, -1, 0, 0, 1, 1, 1};
...
static int full_open_rate_compare(const s32 *full_open, const s32 *cbk, int x, int y, s32 inNodeType, int full_open_rate)
{
	int ret = true;
...
static int codeToOhm(s32 *ohm, const s32 *Code, u16 *v_tdf, u16 *h_tdf)
{
	u32 id = core_mp.chip_id;
	u8 type = core_mp.chip_type;
	...
	for (j = 0; j < core_mp.frame_len; j++) {
		if (Code[j] == 0) {
			...
			temp = ((inTVCH - inTVCL) * inVariation * (inTDF1 - inTDF2) * BIT(12) / (9 * Code[j] * inCint)) * 1000;
...
static int mp_cdc_get_pv5_4_command(u8 *cmd, int len, int index, int frm_idx)
{
	int slen = 0;
	char str[128] = {0};
	...
	/* for P2P I after first frame cmd*/
	if (frm_idx > 0 && tItems[index].catalog == PEAK_TO_PEAK_TEST_I && tItems[index].lcm == ON) {
		key = "noise peak to peak(with panel) i_2";
	} else if (frm_idx > 0 && tItems[index].catalog == PEAK_TO_PEAK_TEST_I && tItems[index].lcm == OFF) {
		key = "noise peak to peak(with panel) i (lcm off)_2";
	}
...
static int mp_cdc_init_cmd_common(u8 *cmd, int len, int index, int frm_idx)
{
	int ret = 0;

	if (core_mp.protocol_ver >= PROTOCOL_VER_540) {
		core_mp.cdc_len = 15;
		return mp_cdc_get_pv5_4_command(cmd, len, index, frm_idx);
	}
...
{
	int i, k, fidx = 0 , ret = 0, len = 0, rd_frame_num = 1, cmd_len = 0, precmd_len = 0;
	u8 cmd[15] = {0};
	u8 precmd[256] = {0};
	u8 *ori = NULL;
	...
	if (tItems[index].catalog == PEAK_TO_PEAK_TEST_I) {
		if (multi_frm_buf == NULL) {
			multi_frm_buf = (s32 **)kzalloc(tItems[index].frame_count * sizeof(s32 *), GFP_KERNEL);
			if (ERR_ALLOC_MEM(multi_frm_buf)) {
				ILI_ERR("Failed to allocate multi_frm_buf mem (%ld)\n", PTR_ERR(multi_frm_buf));
				ret = -EMP_NOMEM;
				goto out;
			}
			for (i = 0; i < tItems[index].frame_count; i++) {
				multi_frm_buf[i] = (s32 *)kzalloc(core_mp.frame_len * sizeof(s32), GFP_KERNEL);
				if (ERR_ALLOC_MEM(multi_frm_buf)) {
					ILI_ERR("Failed to allocate multi_frm_buf[%d] mem (%ld)\n", i, PTR_ERR(multi_frm_buf));
					ret = -EMP_NOMEM;
					goto out;
				}
			}
	...
	if (tItems[index].catalog != PEAK_TO_PEAK_TEST_I)
		fidx =  tItems[index].frame_count-1;

	for (; fidx < tItems[index].frame_count; fidx++) {
		/* Init ori data avoid ori data pollution */
		memset(ori, 0, len);
		...
		/* CDC init */
		if (mp_cdc_init_cmd_common(cmd, sizeof(cmd), index , fidx) < 0) {
		...
		for (k = 0; k < rd_frame_num; k++) {
			...
			for (i = 0; i < (len / 2 - 1); i++) {
				...
				if (tItems[index].catalog == PEAK_TO_PEAK_TEST_I) {
					if ((tmp & 0x8000) == 0x8000)
						multi_frm_buf[fidx][i] = tmp-65536;
					else
						multi_frm_buf[fidx][i] = tmp;
				}
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (frame_buf_len != core_mp.frame_len)	{
		ipio_kfree((void **)&frame_buf);/*free memory before allocate*/
		frame_buf_len = core_mp.frame_len;
	}

	if (frame_buf == NULL) {
		ILI_DBG("Create frame buffer, size: %d\n", frame_buf_len);
		frame_buf = kcalloc(frame_buf_len, sizeof(s32), GFP_KERNEL);
		...
		memset(frame_buf, 0x0, frame_buf_len);
	}
	...
	/* CDC init */
	if (mp_cdc_init_cmd_common(cmd, sizeof(cmd), index , 0) < 0) {
		ILI_ERR("Failed to get cdc command\n");
		ret = -EMP_CMD;
		goto out;
	}
...
static void compare_MaxMin_result(int index, const s32 *data)
{
	int x, y;
...
static int compare_s32(const void *a, const void *b) 
{
    s32 val1 = *(const s32 *)a;
    s32 val2 = *(const s32 *)b;
    return (val1 > val2) - (val1 < val2);
}
static void swap_s32(s32 *a, s32 *b) 
{
    s32 temp = *a;
    *a = *b;
    *b = temp;
...
static void quicksort(void *base, size_t num, size_t size, int (*compar)(const void *, const void *)) 
{
	s32 pivot, *arr;
	size_t left,right;
    ...
    while (left <= right) {
        while (compar(&arr[left], &pivot) < 0) left++;   
        while (compar(&arr[right], &pivot) > 0) right--; 
        
        if (left <= right) {  
            swap_s32(&arr[left], &arr[right]);  
            left++;
            if (right > 0) right--; 
        }
    }
    if (right > 0) quicksort(arr, right + 1, size, compar);
    quicksort(arr + left, num - left, size, compar);
}
static void trimmed_mean_process_frames(int index, int frame_count, int cdc_len, s32 lower_percentage, s32 high_percentage)
{
	int i, j, lower_bound, upper_bound;
	s32 min_val,max_val;
	s32 *sorted_values;

	lower_bound = (frame_count * lower_percentage) / 100;
	upper_bound = frame_count - (frame_count * high_percentage) / 100;

    sorted_values = vmalloc(frame_count * sizeof(s32));
    for(j = 0; j < cdc_len; j++) {
        for (i = 0; i < frame_count; i++) 
            sorted_values[i] = tItems[index].buf[i * cdc_len + j];
        quicksort(sorted_values, frame_count, sizeof(s32), compare_s32);

        min_val = sorted_values[lower_bound];
        max_val = sorted_values[upper_bound - 1];
        tItems[index].diff_buf[j] = (int)(max_val - min_val);
    }
    ipio_vfree((void **)&sorted_values);
}
...
static int create_mp_test_frame_buffer(int index, int frame_count)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].buf == NULL) {
			...
			memset(tItems[index].buf, 0, frame_count * core_mp.frame_len * sizeof(s32));
		...
		if (tItems[index].diff_buf == NULL) {
			tItems[index].diff_buf = kcalloc(core_mp.frame_len, sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(tItems[index].diff_buf)) {
				ILI_ERR("Failed to allocate diff_buf mem\n");
				ipio_kfree((void **)&tItems[index].diff_buf);
				return -ENOMEM;
			}
		}
...
static int peak_to_peak_test(int index)
{
	...
	if (tItems[index].catalog == PEAK_TO_PEAK_TEST_I) {	
		for (i = 0; i < tItems[index].frame_count; i++) {
			for (j = 0; j < cdc_len; j++)
				tItems[index].buf[i * cdc_len + j] = multi_frm_buf[i][j];
			compare_MaxMin_result(index, &tItems[index].buf[i * cdc_len]);
		}
		trimmed_mean_process_frames(index,tItems[index].frame_count,cdc_len,tItems[index].lowest_percentage,tItems[index].highest_percentage);
	} else {
		for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
			for (j = 0; j < cdc_len; j++)
...
int ili_parsing_sram_param(void)
{
	...
	parser_get_u32_array(str, &ilits->sram_para.MaxAddr, 16, sizeof(ilits->sram_para.MaxAddr)/sizeof(u32), ";");

	parser_get_int_data("sram test", "min addr", str, sizeof(str));
	parser_get_u32_array(str, &ilits->sram_para.MinAddr, 16, sizeof(ilits->sram_para.MinAddr)/sizeof(u32), ";");
	...
	if (ilits->sram_para.sram_group_en == ENABLE) {
		...
		strcpy(str_tmp, str);
		ilits->sram_para.write_setting_num = parser_sramtest_group(str, ilits->sram_para.write_info);
		parser_get_u32_array(str_tmp, ilits->sram_para.write_addr, 16, SRAM_TEST_SETTING_LEN, ";");

		parser_get_int_data("sram test", "group write", str, sizeof(str));
		parser_sramtest_group(str, ilits->sram_para.write_info);//[0]=0x291,0x111,0x222  [1]=
		for (i = 0; i < ilits->sram_para.group_num; i++) {
			parser_get_u32_array(ilits->sram_para.write_info[i], ilits->sram_para.SramPartialAreaTest_en[i], 16, SRAM_TEST_SETTING_LEN, ",");
		}

		parser_get_int_data("sram test", "group write len", str, sizeof(str));
		parser_get_u32_array(str, ilits->sram_para.write_addr_len, 10, SRAM_TEST_SETTING_LEN, ";");


		parser_get_int_data("sram test", "group read addr", str, sizeof(str));
		strcpy(str_tmp, str);
		ilits->sram_para.read_setting_num = parser_sramtest_group(str, ilits->sram_para.read_info);
		parser_get_u32_array(str_tmp, ilits->sram_para.read_addr, 16, SRAM_TEST_SETTING_LEN, ";");

		parser_get_int_data("sram test", "group result", str, sizeof(str));
		parser_sramtest_group(str, ilits->sram_para.read_info);//[0]=0x291,0x111,0x222  [1]=
		for (i = 0; i < ilits->sram_para.group_num; i++) {
			parser_get_u32_array(ilits->sram_para.read_info[i], ilits->sram_para.SramResult[i], 16, SRAM_TEST_SETTING_LEN, ",");
		}

		ILI_INFO("Group Number : %d\n", ilits->sram_para.group_num);

		for (i = 0; i < ilits->sram_para.write_setting_num; i++) {
			ILI_INFO("write_addr[%d] = 0x%X, write_addr_len = %d\n", i, ilits->sram_para.write_addr[i], ilits->sram_para.write_addr_len[i]);
		}
		
		for (i = 0; i < ilits->sram_para.read_setting_num; i++) {
			ILI_INFO("read_addr[%d] = 0x%X\n", i, ilits->sram_para.read_addr[i]);
		}

		for (j = 0; j < ilits->sram_para.group_num; j++) {
			for (i = 0; i < ilits->sram_para.write_setting_num; i++) {
				ILI_INFO("Sram Partial Area Test en = 0x%X\n", ilits->sram_para.SramPartialAreaTest_en[j][i]);
			}
			for (i = 0; i < ilits->sram_para.read_setting_num; i++) {
				ILI_INFO("Sram Result = 0x%X\n", ilits->sram_para.SramResult[j][i]);
			}
		}
	}
...
static int mp_compare_test_result(int index)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].catalog == PEAK_TO_PEAK_TEST || tItems[index].catalog == PEAK_TO_PEAK_TEST_I || tItems[index].catalog == TIPRING_P2P) {
				for (i = 0; i < cdclen; i++) {
		...
		/* general result */
		if (tItems[index].trimmed_mean && tItems[index].catalog != PEAK_TO_PEAK_TEST && tItems[index].catalog != PEAK_TO_PEAK_TEST_I) {
		...
		} else if (tItems[index].catalog == PEAK_TO_PEAK_TEST_I ) {
			mp_compare_cdc_result(index, tItems[index].diff_buf, max_threshold, min_threshold, &test_result);
		} else {
			if (tItems[index].bch_mrk_multi) {
				for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
...
out:
	...
	if (multi_frm_buf != NULL) {
		for (i = 0; i < tItems[index].frame_count; i++)
			if (multi_frm_buf[i] != NULL) {
				kfree(multi_frm_buf[i]);
				multi_frm_buf[i] = NULL;
			}
		kfree(multi_frm_buf);
		multi_frm_buf = NULL;
	}
...
static int mp_show_result(bool lcm_on)
{
	...
	for (seq = 0; seq < ri.count; seq++) {
		...
		pr_info("Item : %s, Frame count = %d\n", tItems[i].desp, tItems[i].frame_count);
		...
		if (tItems[i].catalog == RAW_ADAPTABILITY) {
			if (core_mp.nPenChipnum == 0xFF || core_mp.nPenChipnum == 0) {
				ILI_INFO("Pen chip num is not invalid!!\n");
				continue;
			}
		...
		if (threshold_buf_size != core_mp.frame_len) {
			threshold_buf_size = core_mp.frame_len;
			/*free memory before allocate*/
			ipio_kfree((void **)&max_threshold);
			ipio_kfree((void **)&min_threshold);
		}

		if (max_threshold == NULL || min_threshold == NULL) {
			ILI_DBG("Create threshold buf, size is %d\n", threshold_buf_size);

			max_threshold = kcalloc(threshold_buf_size, sizeof(s32), GFP_KERNEL);
			min_threshold = kcalloc(threshold_buf_size, sizeof(s32), GFP_KERNEL);
		...
		if (tItems[i].spec_option == BENCHMARK) {
			if (tItems[i].catalog == PEAK_TO_PEAK_TEST || tItems[i].catalog == PEAK_TO_PEAK_TEST_I) {
				for (j = 0; j < core_mp.frame_len; j++) {
		...
		} else if (tItems[i].catalog == PEAK_TO_PEAK_TEST_I) {
			mp_compare_cdc_show_result(i, tItems[i].diff_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "Trimmed Mean", csv_file_size);
			get_frame_cont = tItems[i].frame_count;
			/* for noise P2P I multiframe */
			for (j = 0; j < get_frame_cont; j++) {
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	ILI_INFO("Firmware AP Version = %x\n", core_mp.fw_ver);
...
static void mp_test_run(int index)
{
	...
	test_result = tItems[i].do_test(i);

	if(test_result < 0){
		tItems[index].item_result = MP_DATA_FAIL;
		ILI_INFO("Run MP Test Item Fail : %s\n", tItems[i].desp);
	} else {
...
static void mp_test_free(void)
{
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		...
		if (tItems[i].catalog == TX_RX_DELTA) {
			...
			ipio_kfree((void **)&tItems[i].diff_buf);
			ipio_kfree((void **)&tItems[i].min_buf);
			ipio_vfree((void **)&tItems[i].buf);
		}
		if (multi_frm_buf != NULL && tItems[i].catalog == PEAK_TO_PEAK_TEST_I) {
			for (j = 0; j < tItems[i].frame_count; j++)
				if (multi_frm_buf[j] != NULL) {
					kfree(multi_frm_buf[j]);
					multi_frm_buf[j] = NULL;
				}
			kfree(multi_frm_buf);
			multi_frm_buf = NULL;
		}
...
int ili_mp_test_main(char *apk, bool lcm_on)
{
	...
	/* Read csv file size */
	ret = parser_get_ini_key_value("pv5_4 command", "csv_file_size", str);
	if (ret < 0) {
		csv_file_size = 3 * M;
	} else {
		csv_read_length = ili_katoi(str);
		if (csv_read_length == 0) {
			csv_file_size = 3 * M;
		} else {
			csv_file_size = csv_read_length + M / 2;
		}
	}
