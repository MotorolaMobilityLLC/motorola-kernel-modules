...
static int ilitek_charger_notifier_callback(struct notifier_block *nb, unsigned long val, void *v)
{
	...
	if (!ret) {
		if((prop.intval == USB_DETECT_IN) || (prop.intval == USB_DETECT_OUT)) {
			...
			ILI_ERR("unsupport prop.intval =%d\n", prop.intval);
	}
	return 0;
}
...
void get_mp_info_log(char *input, int len, int error_code)
{
	/*
	get mp log string by error_code
	*/
	...
	if (error_code == EMP_MODE) {
		snprintf(input, len, "%s\n", "Failed to switch MP mode, abort!");
	} else if (error_code == EMP_FW_PROC) {
		snprintf(input, len, "%s\n", "FW still upgrading, abort!");
	} else if (error_code == EMP_FORMUL_NULL) {
		snprintf(input, len, "%s\n", "MP formula is null, abort!");
	} else if (error_code == EMP_INI) {
		snprintf(input, len, "%s\n", "Not found ini file, abort!");
	} else if (error_code == EMP_NOMEM) {
		snprintf(input, len, "%s\n", "Failed to allocated memory, abort!");
	} else if (error_code == EMP_PROTOCOL) {
		snprintf(input, len, "%s\n", "Protocol version isn't matched, abort!");
	} else if (error_code == EMP_TIMING_INFO) {
		snprintf(input, len, "%s\n", "Failed to get timing info, abort!");
	} else if (error_code == EMP_PARA_NULL) {
...
{
	int ret = 0;
	int recover_cnt = 0, max_retry_cnt = 4;
	...
	{
		recover_cnt ++;
		ILI_INFO("Doing gesture recovery....%d's\n", recover_cnt);
		ret = ilits->ges_recover();

		if (ret < 0)		
			ret = -ret;
		if (ret == EFW_GESRUN_FAIL) {
			/*Reset if gesture run check fail*/
			if (ilits->cascade_info_block.nNum != 0) {
	...
	} while (ret == EFW_GESRUN_FAIL && recover_cnt < max_retry_cnt);
...
int ili_set_tp_data_len(int format, bool send, u8 *data)
{
	...
	case DATA_FORMAT_DEBUG:
		...
		if (ilits->PenType == POSITION_PEN_TYPE_ON) {
			...
			} else if (data_type == P5_X_FW_BG_MODE)  {
				ilits->pen_info.pen_data_mode = HAND_PEN_BG_MODE;
	...
	case DATA_FORMAT_DEBUG_LITE_ALS:
		...
		debug_ctrl = DATA_FORMAT_DEBUG_LITE_ALS_CMD;
		ctrl = DATA_FORMAT_DEBUG_LITE_CMD;
	...
	switch(ilits->pen_info.report_type) {
		...
		case P5_X_ONLY_HAND_TYPE:
			if (type == P5_X_FW_RAW_DATA_MODE) {
				...
				ilits->pen_info.pen_data_mode = HAND_ONLY_BG_MODE;
			} else {
...
int ili_report_handler(void)
{
	...
	case P5_X_DEMO_HIGH_RESOLUTION_PACKET_ID:
		...
		break;
	case P5_X_DEBUG_LITE_A9_PACKET_ID:
		if (trdata[4] == 0x01 || trdata[4] == 0x02 || trdata[4] == 0x03) {
			ili_report_debug_lite_mode(trdata, rlen);
		} else {
			ILI_ERR("This packet ID is %x, but Virtual Key is not valid\n", pid);
		}
		break;
	...
	default:
...
#if (TDDI_INTERFACE == BUS_I2C)
		#if ENABLE_CASCADE
			msleep(ilits->rst_edge_delay + 85);
		#else
			msleep(ilits->rst_edge_delay);
		#endif
		...
		if ((ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE) && ilits->fw_latch != 0) {
			ILI_ERR("I2C esd_gesture recovery by FW\n");
