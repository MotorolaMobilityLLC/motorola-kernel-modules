...
int ili_cascade_rw_tp_reg(u32 mcu, u32 type, u32 addr, u32 wdata, u32 wlen, u32 *rdata, bool bypass_exit_ice)
{
	...
	ret = ilits->ice_mode_ctrl(ENABLE, mcu, BOTH);
	if (ret < 0) {
...
int ili_cascade_rw_tp_reg_v2(u32 mcu, u32 type, u32 addr, u32 wdata, u32 wlen, u32 *rdata)
{
	...
	if (type == ILI_WRITE && ilits->slave_wr_ctrl == SLAVE) {
		...
		len = ili_cascade_rw_tp_reg(mcu, type, addr, wdata, wlen, rdata, false);
	}
...
static ssize_t ilitek_proc_rw_tp_reg_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
    ...
    if (ilits->cascade_info_block.nNum != 0) {
...
#else
		len = ili_cascade_rw_tp_reg(stop_mcu, type, addr, write_data, write_len, &read_data, false);
#endif
#elif (TDDI_INTERFACE == BUS_I2C)
		if (stop_mcu == mcu_on)
			ret = ilits->ice_mode_ctrl(ENABLE, ON, BOTH);
		else
			ret = ilits->ice_mode_ctrl(ENABLE, OFF, BOTH);
		...
		if (stop_mcu == mcu_on)
			ret = ilits->ice_mode_ctrl(DISABLE, ON, BOTH);
		else
			ret = ilits->ice_mode_ctrl(DISABLE, OFF, BOTH);
...
static ssize_t ilitek_proc_fw_cmd_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	if (wrap_wlen > IOCTL_I2C_BUFF || wrap_rlen > IOCTL_I2C_BUFF) {
		ILI_ERR("ERROR! R/W len is larger than ioctl buf\n");
		len = -ENOTTY;
		goto exit;
	} else if (wrap_rlen * 2 + 1 > USER_STR_BUFF) {
		ILI_ERR("ERROR! read len is larger than USER_STR_BUFF\n");
		len = -ENOTTY;
		goto exit;
	}
...
static int ilitek_proc_debug_message_show(struct seq_file *m, void *v)
{
	...
	/* '0' ~ 'F' mapping table */
	static const char charmapping[16] = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46 };
...
static ssize_t ilitek_proc_get_debug_mode_data_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	csv.wlen += snprintf(csv.ptr + csv.wlen, (csv.max_size - csv.wlen), "Get BG data %d frame\n", ilits->bg_count);
	...
	ret = debug_mode_get_data(&csv, P5_X_FW_BG_MODE, ilits->bg_count);
	if (ret < 0)
		goto out;
...
static ssize_t ilitek_node_change_list_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "============= Change list 2025-11-05==============\n");
...
int ili_get_tp_recore_data(bool mcu)
{
	...
	if (!ice)
		ilits->ice_mode_ctrl(DISABLE, ON, BOTH);
...
static int ili_tp_data_mode_ctrl(u8 *cmd)
{
	...
	case DEBUG_LITE_AREA:
		...
		break;
	case DEBUG_LITE_ALS:
		if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_ALS, false, &cmd[1]) < 0) {
			ILI_ERR("Failed to set debug lite als\n");
			ret = -ENOTTY;
		}
		break;	
	case DEBUG_LITE_PEN:
	...
	case DEBUG_LITE_AREA:
		...
		break;
	case DEBUG_LITE_ALS:
		check_debugLite = ili_ic_check_debug_lite_support(DATA_FORMAT_DEBUG_LITE_ALS, false, &cmd[1]);
		if ( check_debugLite == ENABLE) {
			if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_ALS, false, &cmd[1]) < 0) {
				ILI_ERR("Failed to set debug lite als\n");
				ret = -ENOTTY;
			...
			ILI_INFO("no support DEBUG LITE ALS mode, set to debug mode\n");
			cmd[0] = DEBUG_MODE;
			ili_tp_data_mode_ctrl(cmd);
		} else {
			ILI_ERR("change DEBUG LITE ALS fail\n");
		}
		break;
	case DEBUG_LITE_PEN:
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	para = kcalloc(count, sizeof(u32), GFP_KERNEL);
	if (ERR_ALLOC_MEM(para)) {
		ILI_ERR("Failed to allocate data mem\n");
		size = -1;
		goto out;
	}

	memcpy(para, data, count * sizeof(u32));
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		data[1] = 0;
		data[2] = 0;
		memcpy(data, para, count * sizeof(u32));
		ILI_INFO("get panel info : mcu = %x, reset = %x\n", data[1], data[2]);
#if (TDDI_INTERFACE == BUS_I2C)
		ilits->info_from_hex = DISABLE;
#endif

		if (ilits->cascade_info_block.nNum != 0) {
			if (ili_cascade_ic_get_info(true, true, (data[1] != 0), (data[2] != 0)) < 0)
	...
	} else if (strncmp(cmd, "spi_slave_write_mcu_on", strlen(cmd)) == 0) {
		if (data[1] == ENABLE)
			atomic_set(&ilits->spi_slave_write_mcu_on, ENABLE);
		else
			atomic_set(&ilits->spi_slave_write_mcu_on, DISABLE);
		ILI_INFO("spi slave write method mcu = %s\n", atomic_read(&ilits->spi_slave_write_mcu_on) == 1 ? "on" : "off");
...
#endif
		mcu = data[1] & BIT(0);
		ilits->ice_mode_ctrl(DISABLE, mcu, mode);
		...
		if (w_len > sizeof(temp)/sizeof(temp[0])) {
			ILI_ERR("write len is too large\n");
		} else {
			for (i = 0; i < w_len; i++) {
				temp[i] = data[2 + i];
				ILI_INFO("write[%d] = %x\n", i, temp[i]);
		...
		if(r_len > sizeof(temp)/sizeof(temp[0])) {
			ILI_ERR("read len is too large\n");
		} else {
		...
		if((w_len > sizeof(temp)/sizeof(temp[0])) || (r_len > sizeof(temp)/sizeof(temp[0]))) {
			ILI_ERR("length is too large\n");
		} else {
			for (i = 0; i < w_len; i++) {
				temp[i] = data[4 + i];
				ILI_INFO("write[%d] = %x\n", i, temp[i]);
		...
		data[5] = 0;
		memcpy(data, para, count * sizeof(u32));
		ILI_INFO("Get ddi reg one page: page = %x, reg = %x, byFw = %x, mcu = %x, reset = %x\n", data[1], data[2], data[3], data[4], data[5]);
		if (data[3] != 0) {
			ili_ic_get_ddi_reg_by_fw(data[1], data[2], &ddi_data);
		} else {
			if (ilits->cascade_info_block.nNum != 0) {
				ili_ic_get_cascade_ddi_reg_onepage(data[1], data[2], &ddi_data, (data[4] != 0), (data[5] != 0));
			} else {
				ili_ic_get_ddi_reg_onepage(data[1], data[2], &ddi_data, (data[4] != 0), (data[5] != 0));
			}
		}
		ILI_INFO("ddi_data = %x\n", ddi_data);
		...
		data[6] = 0;
		memcpy(data, para, count * sizeof(u32));
		ILI_INFO("Set ddi reg one page: page = %x, reg = %x, data = %x, byFw = %x, mcu = %x, reset = %x\n", data[1], data[2], data[3], data[4], data[5], data[6]);
		if (data[4] != 0) {
			ili_ic_set_ddi_reg_by_fw(data[1], data[2], data[3]);
		} else {
			if (ilits->cascade_info_block.nNum != 0) {
				ili_ic_set_cascade_ddi_reg_onepage(data[1], data[2], data[3], (data[5] != 0), (data[6] != 0));
			} else {
				ili_ic_set_ddi_reg_onepage(data[1], data[2], data[3], (data[5] != 0), (data[6] != 0));
			}
		}
		...
		ILI_INFO("Start = 0x%x, End = 0x%x, Mcu = 0x%x, Reset = 0x%x, Dump Hex path = %s\n", data[1], data[2], data[3], data[4], DUMP_FLASH_PATH);
		ret = ili_fw_dump_flash_data(data[1], data[2], false, (data[3] != 0), (data[4] != 0));
		...
		data[4] = 0;
		memcpy(data, para, count * sizeof(u32));
		ILI_INFO("Start = 0x%x, End = 0x%x, Mcu = 0x%x, Reset = 0x%x, Dump IRAM path = %s\n", data[1], data[2], data[3], data[4], DUMP_IRAM_PATH);
		ilits->fw_update_stat = FW_STAT_INIT;
		ret = ili_fw_dump_iram_data(data[1], data[2], true, (data[3] != 0), (data[4] != 0));
		ilits->fw_update_stat = (ret < 0) ? FW_UPDATE_FAIL : FW_UPDATE_PASS;
		ILI_INFO("ilits->fw_update_stat = %d\n", ilits->fw_update_stat);
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case ILITEK_IOCTL_TP_HW_RESET:
		...
		if (ilits->cascade_info_block.nNum != 0) {
			ili_wdt_reset_status(true, false, false);
		}
		if (ilits->cascade_info_block.nNum != 0) {
			ili_cascade_reset_ctrl(ilits->reset, false);
		} else {
	...
	case ILITEK_IOCTL_ICE_MODE_SWITCH:
		...
		if (ilits->cascade_info_block.nNum != 0) {			
			...
			ilits->ice_mode_ctrl(szBuf[0] ? ENABLE : DISABLE,
				szBuf[1] ? ENABLE : DISABLE, BOTH);
	...
	case ILITEK_IOCTL_TP_DUMP_FLASH:
		...
		ret = ili_fw_dump_flash_data(0, 0, true, OFF, false);
		if (ret < 0) {
	...
	case ILITEK_IOCTL_DDI_WRITE:
		if (copy_from_user(szBuf, (u8 *) arg, 6)) {
			ILI_ERR("Failed to copy data from user space\n");
			ret = -ENOTTY;
			break;
		}
		ILI_DBG("ioctl: page = %x, reg = %x, data = %x, byFw = %x, mcu = %x, reset = %x\n", szBuf[0], szBuf[1], szBuf[2], szBuf[3], szBuf[4], szBuf[5]);
		if (szBuf[3] != 0) {
			ili_ic_set_ddi_reg_by_fw(szBuf[0], szBuf[1], szBuf[2]);
		} else {
			if (ilits->cascade_info_block.nNum != 0) {
				ili_ic_set_cascade_ddi_reg_onepage(szBuf[0], szBuf[1], szBuf[2], (szBuf[4] != 0), (szBuf[5] != 0));
			} else {
				ili_ic_set_ddi_reg_onepage(szBuf[0], szBuf[1], szBuf[2], (szBuf[4] != 0), (szBuf[5] != 0));
			}
		}
		break;
	case ILITEK_IOCTL_DDI_READ:
		if (copy_from_user(szBuf, (u8 *) arg, 5)) {
			ILI_ERR("Failed to copy data from user space\n");
			ret = -ENOTTY;
			break;
		}
		ILI_DBG("ioctl: page = %x, reg = %x, byFw = %x, mcu = %x, reset = %x\n", szBuf[0], szBuf[1], szBuf[2], szBuf[3], szBuf[4]);
		if (szBuf[2] != 0) {
			ili_ic_get_ddi_reg_by_fw(szBuf[0], szBuf[1], &szBuf[5]);
		} else {
			if (ilits->cascade_info_block.nNum != 0) {
				ili_ic_get_cascade_ddi_reg_onepage(szBuf[0], szBuf[1], &szBuf[5], (szBuf[3] != 0), (szBuf[4] != 0));
			} else {
				ili_ic_get_ddi_reg_onepage(szBuf[0], szBuf[1], &szBuf[5], (szBuf[3] != 0), (szBuf[4] != 0));
			}
		}
		ILI_DBG("ioctl: data = %x\n", szBuf[5]);
		if (copy_to_user((u8 *) arg, szBuf + 5, 1)) {
			ILI_ERR("Failed to copy data to user space\n");
			ret = -ENOTTY;
		}
		break;
