...
static int ili_check_panel(void)
{
	...
	if (!active_panel_name)
		ILI_INFO("active_panel NULL\n");
	else if(strstr(active_panel_name, "ili7") || strstr(active_panel_name, "ili98") || strstr(active_panel_name, "il77") || strstr(active_panel_name, "il79"))
	{
...
static int ilitek_spi_probe(struct spi_device *spi)
{
	...
	if (!spi) {
		ILI_ERR("struct pointer is NULL\n");
		return -ENODEV;
	}
	...
	ilits->update_buf = kzalloc(UPDATE_BUF_SIZE, GFP_KERNEL | GFP_DMA);
