	...
	if (!ice) {
		if (reset) {
			if (ilits->cascade_info_block.nNum != 0) {
				if (ili_cascade_reset_ctrl(TP_IC_WHOLE_RST_WITHOUT_FLASH, false))
					ILI_ERR("Reset failed after code reset\n");
			} else {
				if (ili_reset_ctrl(TP_IC_WHOLE_RST_WITHOUT_FLASH))
					ILI_ERR("Reset failed after code reset\n");
			}
		} else {
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		if ((fbi[i].mode & (0x01 << mode)) && fbi[i].len != 0) {
			...
			if (dma_crc_err || dma_crc_err_slave || iram_crc_err) {
					...
					ili_fw_dump_iram_data(0x0, 0xF, false, OFF, false);
					return -EFW_CRC;
			}
		}
	}
...
static int ilitek_tddi_fw_hex_convert(u8 *phex, int size, u8 *pfw)
{
	...
	for (i = 0; i < size;) {
		...
		if (type == 0x04) {
			...
			fbi[num].fix_mem_start = INT_MAX;
			fbi[num].len = fbi[num].end - fbi[num].start + 1;
			ILI_DBG("Block[%d]: start_addr = %x, end = %x", num, fbi[num].start,
				 					fbi[num].end);
