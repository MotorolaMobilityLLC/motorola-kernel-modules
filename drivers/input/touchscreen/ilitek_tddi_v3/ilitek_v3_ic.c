...
static int ili_check_polling_bist(u8 GroupNumIdx, u32* SramRsultStatus, u8 len, u32 mask)
{
	u32 Bist_Sram_Fail = 0xFFFFFFFF;
	int CheckCount = 10, ret = 0;
	u32 i, bist_done = 0, bist_fail = 0;
	...
	if(ilits->sram_para.sram_group_en == ENABLE) {
		if (SramRsultStatus != NULL) {
			if (len < SRAM_TEST_SETTING_LEN) {
				for (i = 0; i < len; i++) {
					ILI_DBG("status read_addr[%d] : 0x%X , Read Sram Partial Area En : 0x%X\n", i, ilits->sram_para.read_addr[i], ilits->sram_para.SramResult[GroupNumIdx][i]);
					ili_ice_mode_read(ilits->sram_para.read_addr[i], &Bist_Sram_Fail, 4);
					ILI_DBG("Bist_Sram_Fail : 0x%X\n", Bist_Sram_Fail);
					if ((Bist_Sram_Fail & SramRsultStatus[i] & mask) > 0) {
						ILI_ERR("Check Bist Fail Flag , Addr[%d] = 0x%X : Fail!\n", i, ilits->sram_para.read_addr[i]);
						ILI_ERR("Bist Sram Fail Status = 0x%X, Sram Rsult Status = 0x%X, mask = 0x%X\n", Bist_Sram_Fail, SramRsultStatus[i], mask);
				...
				ILI_ERR("read_setting_num is out of range!\n");
				ret = -1;
			}
		} else {
			ILI_ERR("Invalid, SramRsultStatus is NULL!\n");
			ret = -1;
		}
...
static int ili_ic_cascade_polling_bist(u8 GroupNumIdx)
{
	int retry = 10, ret = 0, i = 0;
	u32 bist_done = 0, bist_fail = 0;
	u32 bist_slave_done = 0, bist_slave_fail = 0;
	u32* bist_sram_fail_bus = NULL;
	u32* bist_sram_fail_bus_slave = NULL;
#if (TDDI_INTERFACE == BUS_SPI)
	u32 read_back_lock_tmp = 0;
#endif

	if(ilits->sram_para.sram_group_en == ENABLE) {
		bist_sram_fail_bus_slave = kmalloc(ilits->sram_para.read_setting_num * sizeof(u32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(bist_sram_fail_bus_slave)) {
			ILI_ERR("Failed to allocate buf memory, %ld\n", PTR_ERR(bist_sram_fail_bus_slave));
			ret = -ENOMEM;
			goto out;
		}
		bist_sram_fail_bus = kmalloc(ilits->sram_para.read_setting_num * sizeof(u32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(bist_sram_fail_bus)) {
			ILI_ERR("Failed to allocate buf memory, %ld\n", PTR_ERR(bist_sram_fail_bus));
			ret = -ENOMEM;
			goto out;
		}
	}
	...
	ili_ice_mode_read_by_mode(0x04600C, &bist_slave_done, sizeof(bist_slave_done), SLAVE);
	ili_ice_mode_read_by_mode(0x04600D, &bist_slave_fail, sizeof(bist_slave_fail), SLAVE);
	ili_ice_mode_read_by_mode(0x04600C, &bist_done, sizeof(bist_done), MASTER);
	ili_ice_mode_read_by_mode(0x04600D, &bist_fail, sizeof(bist_fail), MASTER);
	...
	if(ilits->sram_para.sram_group_en == ENABLE) {
		for (i = 0; i < ilits->sram_para.read_setting_num; i++) {
			ili_ice_mode_read_by_mode(ilits->sram_para.read_addr[i], &bist_sram_fail_bus_slave[i], sizeof(bist_sram_fail_bus_slave[i]), SLAVE);
			ili_ice_mode_read_by_mode(ilits->sram_para.read_addr[i], &bist_sram_fail_bus[i], sizeof(bist_sram_fail_bus[i]), MASTER);
			ILI_INFO("[SLAVE] ilits->sram_para.read_addr[%d] : 0x%X, bist_sram_fail_bus_slave[%d] : 0x%X\n", i, ilits->sram_para.read_addr[i], i, bist_sram_fail_bus_slave[i]);
			ILI_INFO("[MASTER] ilits->sram_para.read_addr[%d] : 0x%X, bist_sram_fail_bus[%d] : 0x%X\n", i, ilits->sram_para.read_addr[i], i, bist_sram_fail_bus[i]);
			if ((bist_sram_fail_bus_slave[i] & ilits->sram_para.SramResult[GroupNumIdx][i]) > 0) {
				ILI_ERR("[SLAVE] check bist_sram_fail_bus fail, FailValue = 0x%X\n", bist_sram_fail_bus_slave[i] & ilits->sram_para.SramResult[GroupNumIdx][i]);
				ret = -1;
			}
			if ((bist_sram_fail_bus[i] & ilits->sram_para.SramResult[GroupNumIdx][i]) > 0) {
				ILI_ERR("[MASTER] check bist_sram_fail_bus fail, FailValue = 0x%X\n", bist_sram_fail_bus[i] & ilits->sram_para.SramResult[GroupNumIdx][i]);
				ret = -1;
			}
		}
	} else {
		if (((bist_fail & BIT(0)) != 0x00) || ((bist_slave_fail & BIT(0)) != 0x00)) {
			ILI_ERR("SRAM TEST FAIL\n");
	...
	ili_ice_mode_read(SLAVE_READ_BACK_WORD_REG, &read_back_lock_tmp, sizeof(read_back_lock_tmp));
	bist_slave_done = ili_swap32(read_back_lock_tmp);

	ili_ice_mode_write(READ_BACK_LOCK_REG, 0x01, 1);
	ili_ice_mode_read(0x04600D, &bist_fail, sizeof(bist_fail));
	ili_ice_mode_read(SLAVE_READ_BACK_WORD_REG, &read_back_lock_tmp, sizeof(read_back_lock_tmp));
	bist_slave_fail = ili_swap32(read_back_lock_tmp);
	...
	if(ilits->sram_para.sram_group_en == ENABLE) {
		for (i = 0; i < ilits->sram_para.read_setting_num; i++) {
			ili_ice_mode_write(READ_BACK_LOCK_REG, 0x01, 1);
			ili_ice_mode_read(ilits->sram_para.read_addr[i], &bist_sram_fail_bus[i], sizeof(bist_sram_fail_bus[i]));
			ili_ice_mode_read(SLAVE_READ_BACK_WORD_REG, &read_back_lock_tmp, sizeof(read_back_lock_tmp));
			bist_sram_fail_bus_slave[i] = ili_swap32(read_back_lock_tmp);
			ILI_INFO("[SLAVE] ilits->sram_para.read_addr[%d] : 0x%X, bist_sram_fail_bus_slave[%d] : 0x%X\n", i, ilits->sram_para.read_addr[i], i, bist_sram_fail_bus_slave[i]);
			ILI_INFO("[MASTER] ilits->sram_para.read_addr[%d] : 0x%X, bist_sram_fail_bus[%d] : 0x%X\n", i, ilits->sram_para.read_addr[i], i, bist_sram_fail_bus[i]);
			if ((bist_sram_fail_bus_slave[i] & ilits->sram_para.SramResult[GroupNumIdx][i]) > 0) {
				ILI_ERR("[SLAVE] check bist_sram_fail_bus fail, FailValue = 0x%X\n", bist_sram_fail_bus_slave[i] & ilits->sram_para.SramResult[GroupNumIdx][i]);
...
#endif
out:
	if (bist_sram_fail_bus != NULL)
		ipio_kfree((void **)&bist_sram_fail_bus);
	if (bist_sram_fail_bus_slave != NULL)
		ipio_kfree((void **)&bist_sram_fail_bus_slave);
	return ret;
}
...
static int ili_sram_by_partial_area_test(void)
{
	...
	for(nSramNumIdx = 0; nSramNumIdx < ilits->sram_para.group_num; nSramNumIdx++) {
		ILI_INFO("============ Group No : %d , Sram Partial Area : %s ============\n", nSramNumIdx, ilits->sram_para.strSramPartialArea[nSramNumIdx]);
		for(i = 0; i < ilits->sram_para.write_setting_num; i++) {
			ili_ice_mode_write_by_mode(ilits->sram_para.write_addr[i], ilits->sram_para.SramPartialAreaTest_en[nSramNumIdx][i], ilits->sram_para.write_addr_len[i], BOTH);
			ILI_INFO("write_addr : 0x%X , write_addr_len : %d , Write Sram Partial Area En : 0x%X\n", ilits->sram_para.write_addr[i], ilits->sram_para.write_addr_len[i], ilits->sram_para.SramPartialAreaTest_en[nSramNumIdx][i]);
		}
		ili_sram_test(ilits->sram_para.MaxAddr, ilits->sram_para.MinAddr);

		if (ilits->cascade_info_block.nNum != 0) {
			if (ili_ic_cascade_polling_bist(nSramNumIdx) < 0) {
				ILI_ERR("Check March_LR Bist Fail during partial area test\n");
				ret = -1;
			}
		} else {
			if (ili_check_polling_bist(nSramNumIdx, ilits->sram_para.SramResult[nSramNumIdx], ilits->sram_para.read_setting_num, 0xFFFFFFFF) < 0) {
				ILI_ERR("Check March_LR Bist Fail during partial area test\n");
				ret = -1;
			}
		}
...
static int ili_IL79451A_sram_test(void)
{
	...
	for (nSramNumIdx = 0; nSramNumIdx < ilits->sram_para.group_num; nSramNumIdx++) {
		for (i = 0; i < 2; i++) {
			...
			for (j = 0; j < ilits->sram_para.write_setting_num; j++) {
				/* 0x04603C 0x65 or 0x04603C 0x1E*/
				ili_ice_mode_write_by_mode(ilits->sram_para.write_addr[i], ilits->sram_para.SramPartialAreaTest_en[nSramNumIdx][i], 4, BOTH);
				ILI_DBG("Write Sram Partial Area En : 0x%X\n", ilits->sram_para.SramPartialAreaTest_en[nSramNumIdx][i]);
			}
			...
			if (ili_check_polling_bist(nSramNumIdx, ilits->sram_para.SramResult[nSramNumIdx], ilits->sram_para.read_setting_num, mask[nSramNumIdx][i]) < 0) {
				ILI_ERR("Check March_LR Bist Fail during partial area test\n");
				ret = -1;
				goto out;
			}
...
int ili_tddi_ic_sram_test()
{
	...
	if (ilits->cascade_info_block.nNum != 0) {
		ret = ili_ic_cascade_polling_bist(false);
	} else {
		ret = ili_check_polling_bist(false, NULL, 0, 0xFFFFFFFF);
	}
out:
#if (TDDI_INTERFACE == BUS_SPI)
	if (ilits->cascade_info_block.nNum != 0) {
...
int ili_ice_mode_read_by_mode(u32 addr, u32 *data, int len, int mode)
{
	...
	switch (mode) {
	case SLAVE:
		if (ili_spi_ice_mode_read(addr, data, len, SLAVE) < 0) {
			ILI_ERR("Failed to read Slave, addr: 0x%x\n", addr);
			ret = -1;
		} else {
			ILI_DBG("Slave [READ]:addr = 0x%06x, read = 0x%08x\n", addr, *data);
...
int ili_ice_slave_write_register(u32 addr, u32 data, int len)
{
	...
	if (mcu) {
		ice_header_addr = ICE_HEADER_REG + 0x03; /* (ICE_HEADER_REG + 0x03) is nonstop enter ice mode setting*/
		ice_head = ICE_MCU_ON_HEADER;
	} else {
		ice_header_addr = ICE_HEADER_REG; /* (ICE_HEADER_REG) is stop enter ice mode setting*/
		ice_head = ICE_HEADER;
	}
...
#if ENABLE_SPICASCADE_V2
static int ili_spi_ms_register_read(u32 addr, u32 *data, int len, u8 msmode)
{
	int ret = 0;
...
int ili_set_bypass_mode(bool mcu)
{
	...
	/* set s_to_q bit[1] = 1 */
	//ili_ice_mode_write(INGLE_TO_QUAL_REG, 0x02, 1);
...
int ili_ic_set_ddi_reg_by_fw(u8 page, u8 reg, u8 val)
{
	int ret = 0;
	u8 cmd[4];
	cmd[0] = P5_X_SET_DDI_STATUS;
	cmd[1] = page;
	cmd[2] = reg;
	cmd[3] = val;
	...
	if (ret < 0) {
		ILI_ERR("Set ddi status failed\n");
		return -EINVAL;
	}
	return ret;
}

int ili_ic_get_ddi_reg_by_fw(u8 page, u8 reg, u8 *data)
{
	int ret = 0;
	u8 cmd[3], buf[4];
	cmd[0] = P5_X_GET_DDI_STATUS;
	cmd[1] = page;
	cmd[2] = reg;

	ret = ilits->wrapper(cmd, sizeof(cmd), buf, 4, ON, OFF);

	if (ret < 0) {
		ILI_ERR("Get ddi status failed\n");
		return -EINVAL;
	}
...
void ili_ic_set_cascade_ddi_reg_onepage(u8 page, u8 reg, u8 data, bool mcu, bool reset)
{
...
void ili_ic_get_cascade_ddi_reg_onepage(u8 page, u8 reg, u8 *data, bool mcu, bool reset)
{
	int ret = 0;
	u32 setpage = 0x1FFFFF00 | page;
	...
	if (ilits->chip->id == ILI79900) {
	    /* reduce clk, otherwise Polling 0x4800A Read done will fail
	       5 bit, 0x01~0x1F, */
	    if (ili_ice_mode_read(0x73008, &reg_data, sizeof(u32)) < 0) {
	        ILI_ERR("Failed to read from 0x73008\n");
	    }
	    if (ili_ice_mode_write(0x73008, (reg_data | 0x01), sizeof(u32)) < 0) {
	        ILI_ERR("Failed to write to 0x73008\n");
	    }
	}
...
void ili_ic_set_ddi_reg_onepage(u8 page, u8 reg, u8 data, bool mcu, bool reset)
{
	u32 setpage = 0x1FFFFF00 | page;
	u32 setreg = 0x1F000100 | (reg << 16) | data;
	bool ice = atomic_read(&ilits->ice_stat);
	...
	if (!ice) {
		if (reset) {
			if (ili_reset_ctrl(ilits->reset) < 0)
				ILI_ERR("Reset failed after writing ddi reg\n");
		} else {
...
void ili_ic_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data, bool mcu, bool reset)
{
	u32 setpage = 0x1FFFFF00 | page;
	u32 setreg = 0x2F000100 | (reg << 16);
	u32 reg_data = 0;
	...
	*data = ilitek_tddi_ic_rd_pack(setreg);
	...
	if (!ice) {
		if (reset) {
			if (ili_reset_ctrl(ilits->reset) < 0)
				ILI_ERR("Reset failed after reading ddi reg\n");
		} else {
			if (ili_ice_mode_ctrl(DISABLE, mcu) < 0)
...
void ili_ic_get_pc_counter(int stat)
{
	...
	if (ilits->cascade_info_block.nNum != 0) {
		...
		ili_cascade_rw_tp_reg(mcu_state, ILI_READ, pc_addr, 0, 0, &pc, true);
		ili_cascade_rw_tp_reg(mcu_state, ILI_READ, latch_addr, 0, 0, &latch, true);
		...
		if (!ice) {
			if (ilits->ice_mode_ctrl(DISABLE, ON, BOTH) < 0)
...
out:
	ILI_INFO("Firmware AP Version = %d.%d.%d.%d\n", buf[1], buf[2], buf[3], buf[4]);
...
int ili_ic_get_panel_info(void)
{
	...
	do {
		...
		if (buf[0] != cmd) {
			...
			ilits->cascade_info_block.nNum = CASCADE_NUM;
		} else {
...
int ili_ic_get_info(void)
{
	...
	if (((ilits->chip->pid >> 28) & 0xF) == 0xF) {
		...
		if (ilits->chip->id == ILI9882_CHIP && ilits->chip->type == ILI_N) {
			ili_ic_get_ddi_reg_onepage(0x6, 0xF3, &ddi_data, OFF, false);
			if (ddi_data == 0x30)
...
{
	u8 cmd[12] = {0}, buf[4] = {0}, ctrl = 0, data_type = 0;
	int ret;
	if (ilits->chip->core_ver < CORE_VER_1700) {
	...
	case DATA_FORMAT_DEBUG_LITE_AREA:
		...
		cmd[7] = data[4];
		break;
	case DATA_FORMAT_DEBUG_LITE_ALS:
		if (data == NULL) {
			ILI_ERR("DATA_FORMAT_DEBUG_LITE_ALS error cmd\n");
			return -1;
		}
		ctrl = DATA_FORMAT_DEBUG_LITE_CMD;
		data_type = data[0];
