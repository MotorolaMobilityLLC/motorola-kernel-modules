...
#define TIMEOUT_SECTOR		500 //sector erase
#define TIMEOUT_PAGE		3500 //Block erase 64k
#define TIMEOUT_PROGRAM		50 //program
#define TIMEOUT_PROTECT 	100 //write
...
static u32 HexToDec(const char *phex, s32 len)
{
	u32 ret = 0, temp = 0, i;
	s32 shift = (len - 1) * 4;
...
static int CalculateCRC32(u32 start_addr, u32 len,const u8 *pfw)
{
	int i = 0, j = 0;
...
int ili_fw_dump_iram_data(u32 start, u32 end, bool save, bool mcu, bool reset)
{
	...
	if (ilits->ice_mode_ctrl(ENABLE, mcu, BOTH) < 0) {
...
{
	if (ili_ice_mode_bit_mask_write(FLASH4_ADDR, FLASH4_reg_flash_dma_trigger_en, 0) < 0)
		ILI_ERR("Mask write: mask=%lu, value=%d, addr=0x%x failed\n", FLASH4_reg_flash_dma_trigger_en, 0, FLASH4_ADDR);

	if (ili_ice_mode_bit_mask_write(FLASH0_ADDR, FLASH0_reg_rx_dual, 0) < 0)
		ILI_ERR("Mask write: mask=%lu, value=%d, addr=0x%x failed\n", FLASH0_reg_rx_dual, 0, FLASH0_ADDR);
...
{
	if (ili_ice_mode_bit_mask_write_cascade(FLASH4_ADDR, FLASH4_reg_flash_dma_trigger_en, 0, BOTH) < 0)
		ILI_ERR("Cascade mask write: mask=%lu, value=%d, addr=0x%x failed\n", FLASH4_reg_flash_dma_trigger_en, 0, FLASH4_ADDR);

	if (ili_ice_mode_bit_mask_write_cascade(FLASH0_ADDR, FLASH0_reg_rx_dual, 0, BOTH) < 0)
		ILI_ERR("Cascade mask write: mask=%lu, value=%d, addr=0x%x failed\n", FLASH0_reg_rx_dual, 0, FLASH0_ADDR);
...
int ili_fw_dump_flash_data(u32 start, u32 end, bool user, bool mcu, bool reset)
{
...
static void ilitek_tddi_flash_read(u32 *data, u8 cmd, int len)
{
	u32 rxbuf[4] = {0};
	int i = 0;
...
static void ilitek_tddi_flash_write(u8 cmd, u8 *sendbuf,int len)
{
	int i = 0;
	if (ili_ice_mode_write(FLASH_BASED_ADDR, 0x0, 1) < 0)/* CS Low */
		ILI_ERR("Write cs low failed\n");
...
int ilitek_tddi_flash_protect(bool enable, bool mcu)
{
	...
	if (ilits->isSupportFlash) {
		...
		for (i = 0; i < flash_protect_list[ilits->supportFlashIndex].writeOPCount; i++) {
			...
			if (ilitek_tddi_flash_poll_busy(flash_protect_list[ilits->supportFlashIndex].polltime_write) < 0)
...
static int ilitek_fw_judge_hex(u8 *pfw, u32 *customerHexAddrStart, u32 *customerHexAddrEnd, u32 *mpHexAddrStart, u32 *mpHexAddrEnd)
{
...
static int ilitek_fw_check_ddi_chunk(u8 *pfw)
{
	...
	if (block_info[0] == P5_X_GET_BLOCK_INFOMATION && ret == UPDATE_PASS) {
		...
		ILI_INFO("Block info is supported by hex, but the current FW is not.\n");
		...
		customerFlashAddrStart = customerHexAddrStart;
		customerFlashAddrEnd = customerHexAddrEnd;
		mpFlashAddrStart = mpHexAddrStart;
		mpFlashAddrEnd = mpHexAddrEnd;
		customerFlashSize = customerFlashAddrEnd - customerFlashAddrStart + 1;
		mpFlashSize = mpFlashAddrEnd - mpFlashAddrStart + 1;
	...
	if (check_fw_ver) {
		...
		if (ilits->force_fw_update) {
			if (tfd.new_fw_cb != ilits->chip->fw_ver) {
				ILI_INFO("Firmware AP Version not matched\n");
				return UPDATE_FAIL;
			}
		} else {
...
static int ilitek_tddi_fw_flash_program(const u8 *pfw)
{
	int ret = UPDATE_PASS;
	u8 *buf;
	u32 i = 0, j = 0, addr = 0, k = 0, recv_addr = 0, addr_base = 0, data_len = 0;
	int page = ilits->program_page;
	int flash_area = 0;
	int start_area = 0, end_area = 0;
	int start_idx = 0, end_idx = 0;
	int progress_total = FW_BLOCK_INFO_NUM + FW_BLOCK_INFO_B1_NUM;
	bool skip = true;

	buf = kmalloc((page + 4) * sizeof(u8), GFP_DMA);
	if (ERR_ALLOC_MEM(buf)) {
		ILI_ERR("Failed to allocate buf memory, %ld\n", PTR_ERR(buf));
		ret = -ENOMEM;
		goto out;
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		start_area = fbi[i].start / page;
		end_area = fbi[i].end / page;

		ILI_DBG("[AF INITIAL] start_area = %d, end_area = %d\n", start_area, end_area);

		for (flash_area = start_area; flash_area <= end_area; flash_area++) {
			addr_base = flash_area * page;
			if (flash_area == start_area) {
				start_idx = fbi[i].start % page;
			} else {
				start_idx = 0;
			}
			if (flash_area == end_area) {
				end_idx = fbi[i].end % page;
			} else {
				end_idx = (page - 1);
			}
			ILI_DBG("[flash_area = %d, area_base_addr = 0x%x] start_area = %d, end_area = %d, start_idx = %d, end_idx = %d\n",
				flash_area, addr_base, start_area, end_area, start_idx, end_idx);
			buf[0] = 0x25;
			buf[3] = 0x04;
			buf[2] = 0x10;
			buf[1] = 0x08;
			...
			if (skip) {
				ILI_DBG("This round is all 0xFF, skip\n");
				continue;
			}
			...
			if (ilits->wrapper(buf, data_len + 4, NULL, 0, OFF, OFF) < 0) {
			...
			if (ilits->flash_mid == 0xEF) {
				...
				if (ilitek_tddi_flash_poll_busy(flash_protect_list[ilits->supportFlashIndex].polltime_program) < 0) {
					ret = UPDATE_FAIL;
					goto out;
				}
			}
		...
		j++;
		if (ilits->fw_update_stat != ((j * 100) / progress_total)) {
			ilits->fw_update_stat = (j * 100) / progress_total;
			ILI_DBG("Program flash data .... %d%c\n", ilits->fw_update_stat, '%');
		}
	}
	for (i = 0; i < FW_BLOCK_INFO_B1_NUM; i++) {
		...
		start_area = fbi_b1[i].start / page;
		end_area = fbi_b1[i].end / page;

		ILI_DBG("[B1 INITIAL] start_area = %d, end_area = %d\n", start_area, end_area);

		for (flash_area = start_area; flash_area <= end_area; flash_area++) {
			addr_base = flash_area * page;
			if (flash_area == start_area) {
				start_idx = fbi_b1[i].start % page;
			} else {
				start_idx = 0;
			}
			if (flash_area == end_area) {
				end_idx = fbi_b1[i].end % page;
			} else {
				end_idx = (page - 1);
			}
			ILI_DBG("[flash_area = %d, area_base_addr = 0x%x] start_area = %d, end_area = %d, start_idx = %d, end_idx = %d\n",
	...
	ilits->fw_update_stat = 100;
	ILI_DBG("Program flash data .... 100%% done\n");
out:
	ipio_kfree((void **)&buf);
	return ret;
}
...
static int ilitek_tddi_fw_flash_erase(bool mcu)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr <= fbi[i].end; addr += ilits->flash_sector) {
			...
			if (addr == fbi[AP].start)
				ret = ilitek_tddi_flash_poll_busy(flash_protect_list[ilits->supportFlashIndex].polltime_erase64K);
			else
				ret = ilitek_tddi_flash_poll_busy(flash_protect_list[ilits->supportFlashIndex].polltime_erase);
	...
	if (!ice) {
		if (ilits->ice_mode_ctrl(DISABLE, mcu, BOTH) < 0)
...
static int ilitek_fw_flash_upgrade(u8 *pfw, bool mcu)
{
	...
	/* Check FW version */
	ILI_INFO("New Firmware AP Version = 0x%x, Current Firmware AP Version = 0x%x, New Firmware MP Version = 0x%x, Current Firmware MP Version = 0x%x\n",
	...
	if (ret == UPDATE_PASS) {
		...
		if (ilits->cascade_info_block.nNum != 0) {
			if (ili_cascade_reset_ctrl(ilits->reset, false) < 0) {
				ILI_ERR("TP reset failed during fast enter ice mode\n");
				return -EFW_REST;
			}
		} else {
			if (ili_reset_ctrl(ilits->reset) < 0) {
...
static void ilitek_tddi_fw_update_block_info(u8 *pfw)
{
	...
	ILI_INFO("New Firmware AP Version = 0x%x, New Firmware MP Version = 0x%x,\n", tfd.new_fw_cb, tfd.new_mp_fw_cb);
...
out:
	...
	ILI_INFO("Flash timeout spec : write=%d, program=%d, erase=%d, erase64K=%d, chip erase=%d\n"
			 , flash_protect_list[ilits->supportFlashIndex].polltime_write
			 , flash_protect_list[ilits->supportFlashIndex].polltime_program
			 , flash_protect_list[ilits->supportFlashIndex].polltime_erase
			 , flash_protect_list[ilits->supportFlashIndex].polltime_erase64K
			 , flash_protect_list[ilits->supportFlashIndex].polltime_chip_erase);
}