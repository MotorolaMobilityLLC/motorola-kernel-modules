...
#define ESWIN_LOG_DEVICE_NAME "tp_tools"
...
static struct eswin_ts_log ts_log_dev;
...
{
    struct eswin_ts_log *log = file->private_data;
...
static int eswin_log_device_open(struct inode *inode, struct file *filp)
{
...
int eswin_log_capture_register_misc(struct eph_data *ephdata)
{
    ...
    mutex_init(&ephdata->frame_log_lock);

    ts_log_dev.miscdev.minor = MISC_DYNAMIC_MINOR;
    ts_log_dev.miscdev.name = ESWIN_LOG_DEVICE_NAME;
...
int eswin_log_capture_unregister_misc(struct eph_data *ephdata)
{
    ...
    mutex_destroy(&ephdata->frame_log_lock);
    return 0;
}
