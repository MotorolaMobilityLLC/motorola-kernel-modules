...
int ili_move_gesture_code_flash(int mode)
{
	...
	ret = ili_set_tp_data_len(mode, true);
...
int ili_move_gesture_code_iram(int mode)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		ret = ili_ic_func_ctrl("lpwg", 0x5);
		if (ret < 0) {
			ILI_ERR("write check load code error");
			goto out;
		}

		ret = ilits->wrapper(NULL, 0, cmd, sizeof(u8), ON, OFF);
		if (ret < 0) {
...
int ili_touch_esd_gesture_flash(void)
{
	...
	ILI_INFO("ESD Gesture PWD Addr = 0x%X, PWD = 0x%X\n",
		I2C_ESD_GESTURE_PWD_ADDR, ESD_GESTURE_PWD);

	/* write a special password to inform FW go back into gesture mode */
	ret = ili_ice_mode_write(I2C_ESD_GESTURE_PWD_ADDR, ESD_GESTURE_PWD, 4);
	if (ret < 0) {
	...
	do {
		ret = ili_ice_mode_read(I2C_ESD_GESTURE_PWD_ADDR, &answer, sizeof(u32));
		if (ret < 0) {
...
out:
	...
	if (ret >= 0) {
		...
		ili_set_tp_data_len(ilits->gesture_mode, false);
	}
...
int ili_touch_esd_gesture_iram(void)
{
	...
	/* write a special password to inform FW go back into gesture mode */
	ret = ili_ice_mode_write(esd_ges_pwd_addr, ESD_GESTURE_PWD, 4);
	if (ret < 0) {
		ILI_ERR("write password failed\n");
		goto fail;
	}
	...
	/* polling another specific register to see if gesutre is enabled properly */
	do {
		ret = ili_ice_mode_read(esd_ges_pwd_addr, &answer, sizeof(u32));
		if (ret < 0) {
			ILI_ERR("Read gesture answer error\n");
			goto fail;
		}
