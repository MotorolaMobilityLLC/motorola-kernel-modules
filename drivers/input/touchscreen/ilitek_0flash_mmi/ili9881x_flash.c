...
static int ilitek_fw_flash_hex_hw_crc(u8 *pfw)
{
	u32 i = 0, len = 0;
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		hw_crc = ili_fw_read_hw_crc(fbi[i].start, len);

		ILI_INFO("Block = %d, Hex CRC = %x, HW CRC = %x\n", i, hex_crc, hw_crc);
...
static int ilitek_tddi_fw_flash_program(u8 *pfw)
{
	...
	for (i = 0; i < FW_BLOCK_INFO_NUM; i++) {
		...
		for (addr = fbi[i].start; addr < fbi[i].end; addr += page) {
			...
			ILI_DBG("Reading flahse data .... %d%c", ilits->fw_update_stat, '%');
		}
	}
	return UPDATE_PASS;
}
...
static int ilitek_fw_flash_upgrade(u8 *pfw)
{
	...
	ret = ilitek_fw_flash_hex_hw_crc(pfw);
	if (ret == UPDATE_FAIL)
		return -EFW_CRC;
