...
static int ilitek_tddi_ic_check_support(u32 pid, u16 id)
{
	...
	ilits->chip->reset_key = 0x00019881;
...
int ili_ice_mode_ctrl(bool enable, bool mcu)
{
	...
	if (enable) {
		...
		do {
			...
			if (ilits->spi_speed != NULL && ilits->chip->spi_speed_ctrl)
				ilits->spi_speed(ON);

			/* Read chip id to ensure that ice mode is enabled successfully */
			if (ili_ice_mode_read(ilits->chip->pid_addr, &pid, sizeof(u32)) < 0)
...
int ili_ic_watch_dog_ctrl(bool write, bool enable)
{
	...
	if (ilits->chip->wdt_addr <= 0 || ilits->chip->id <= 0) {
		ILI_ERR("WDT/CHIP ID is invalid\n");
		return -EINVAL;
	}

	/* FW will automatiacally disable WDT in I2C */
	if (ilits->wtd_ctrl == OFF) {
		ILI_INFO("WDT ctrl is off, do nothing\n");
		return 0;
	}

	if (!write) {
		if (ili_ice_mode_read(ilits->chip->wdt_addr, &ret, sizeof(u8)) < 0) {
	...
	ILI_INFO("%s WDT, key = %x\n", (enable ? "Enable" : "Disable"), ilits->chip->wtd_key);

	if (enable) {
		if (ili_ice_mode_write(ilits->chip->wdt_addr, 1, 1) < 0)
			ILI_ERR("Wrie WDT key failed\n");
	} else {
		/* need delay 300us to wait fw relaod code after stop mcu. */
		udelay(300);
		if (ili_ice_mode_write(ilits->chip->wdt_addr, (ilits->chip->wtd_key & 0xff), 1) < 0)
			ILI_ERR("Write WDT key failed\n");
		if (ili_ice_mode_write(ilits->chip->wdt_addr, (ilits->chip->wtd_key >> 8), 1) < 0)
			ILI_ERR("Write WDT key failed\n");
	}
	...
	while (timeout > 0) {
		...
		if (enable) {
			...
			if (ili_ice_mode_write(ilits->chip->wdt_addr, 0x00, 1) < 0)
				ILI_ERR("Write 0x0 at %x\n", ilits->chip->wdt_addr);
			if (ili_ice_mode_write(ilits->chip->wdt_addr, 0x98, 1) < 0)
	...
	if (enable) {
		...
		if (ili_ice_mode_write(ilits->chip->wdt_addr, 0, 1) < 0)
...
int ili_ic_func_ctrl(const char *name, int ctrl)
{
	...
	ret = ilits->wrapper(func_ctrl[i].cmd, func_ctrl[i].len, NULL, 0, ON, OFF);
	if (ret < 0)
...
int ili_ic_check_int_pulse(bool pulse)
{
	...
	if (!wait_event_interruptible_timeout(ilits->inq, !atomic_read(&ilits->cmd_int_check), msecs_to_jiffies(timer))) {
		ILI_ERR("Error! INT pulse no detected.\n");
...
int ili_ic_check_busy(int count, int delay)
{
	...
	do {
		if (ilits->wrapper(cmd, sizeof(cmd), NULL, 0, ON, OFF) < 0)
...
int ili_ic_get_core_ver(void)
{
	...
	do {
		if (i == 0) {
			...
			len = ilits->protocol->core_ver_len + 1;
		} else {
			cmd[0] = P5_X_READ_DATA_CTRL;
		...
		if (ilits->wrapper(&cmd[1], sizeof(u8), buf, len, ON, OFF) < 0)
