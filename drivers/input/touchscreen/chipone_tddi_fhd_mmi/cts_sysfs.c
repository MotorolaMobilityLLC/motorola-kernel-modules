...
#define PRINT_ROW_SIZE          (16)
	...
	if ((cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911C) ||
		...
		for (i = 0; i < size; i++) {
			ret =
			    cts_fw_reg_writel_retry(cts_dev, HW_STUB_ADDR,
						    addr + i, 3, 10);
			if (ret) {
				count =
			...
			ret =
			    cts_fw_reg_readsb(cts_dev, HW_STUB_ADDR + 4,
					      data + i, 1);
			if (ret) {
				count =
				    snprintf(buf, PAGE_SIZE,
	...
	if ((cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9916) ||
		...
		if (cts_dev->rtdata.program_mode) {
			for (i = 0; i < size; i++) {
				ret =
				    cts_dev_readb(cts_dev, addr + i, data + i,
						  3, 10);
				if (ret) {
					count =
					    snprintf(buf, PAGE_SIZE,
			...
			ret =
			    cts_dev->ops->read_hw_reg(cts_dev, addr, data,
						      size);
			if (ret < 0) {
				count =
				    snprintf(buf, PAGE_SIZE,
				  ...
				  const char *buf, size_t count)
{
	...
	if ((cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9911C) ||
		...
		for (i = 0; i < argc - 1; i++) {
			...
			ret =
			    cts_fw_reg_writeb_retry(cts_dev, HW_STUB_ADDR + 4,
						    data[i], 3, 10);
			if (ret) {
				cts_err("Write hw register error");
				break;
	...
	if ((cts_dev->hwdata->hwid == CTS_DEV_HWID_ICNL9916) ||
		...
		if (cts_dev->rtdata.program_mode) {
			for (i = 0; i < argc - 1; i++) {
				ret =
				    cts_dev_writeb(cts_dev, addr + i, data[i],
						   3, 10);
				if (ret) {
					cts_err("Write hw register error");
					break;
			...
			ret =
			    cts_dev->ops->write_hw_reg(cts_dev, addr, data,
						       argc - 1);
			if (ret < 0)
				cts_err("Write hw register error");
		}
		...
		if (data_valid) {
			...
			for (r = 0; r < cts_dev->fwdata.rows; r++) {
				for (c = 0; c < cts_dev->fwdata.cols; c++) {
					...
					rawdata[r * cts_dev->hwdata->num_col +
						c] -=
					    manualdiff_base[r *
							    cts_dev->hwdata->num_col + c];
					val =
					    rawdata[r *
						    cts_dev->hwdata->num_col +
						    c];
				    ...
				    const char *buf, size_t count)
{
	...
	if (strncasecmp("updatebase", argv[0], 10) == 0) {
		...
		if (!manualdiff_base && (DIFFDATA_BUFFER_SIZE(cts_dev) > 0)) {
			manualdiff_base =
			    (s16 *) kzalloc(DIFFDATA_BUFFER_SIZE(cts_dev),
					    GFP_KERNEL);
			if (manualdiff_base == NULL) {
				cts_err("Malloc manualdiff_base failed");
				goto err_manual_diff_store;
