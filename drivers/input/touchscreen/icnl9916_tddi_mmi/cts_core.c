        ...
        const u8 *src, size_t len, int retry, int delay)
{
    ...
    while (len) {
        payload_len = min((size_t) (max_xfer_size - cts_dev->rtdata.addr_width),
            len);
        ...
        u32 addr, void *dst, size_t len, int retry, int delay, int idle)
{
    ...
    while (len) {
        ...
        ret = cts_plat_spi_read_delay_idle(cts_dev->pdata,
            cts_dev->rtdata.slave_addr, addr_buf, cts_dev->rtdata.addr_width,
        ...
        u32 addr, u32 *l, int retry, int delay)
{
    ...
    if (cts_dev->rtdata.program_mode) {
        ...
        ret =
            cts_read_sram_normal_mode(cts_dev, addr, buff, 4, retry, delay);
        if (ret) {
...
static void tsdata_flip_x(void *tsdata, u8 fw_rows, u8 fw_cols)
{
    ...
    for (r = 0; r < fw_rows; r++) {
        for (c = 0; c < fw_cols / 2; c++) {
            swap(data[r * fw_cols + c], data[r * fw_cols + wrap(fw_cols, c)]);
        }
    }
}
...
static void tsdata_flip_y(void *tsdata, u8 fw_rows, u8 fw_cols)
{
    ...
    for (r = 0; r < fw_rows / 2; r++) {
        for (c = 0; c < fw_cols; c++) {
            swap(data[r * fw_cols + c], data[wrap(fw_rows, r) * fw_cols + c]);
        }
    }
}
...
//read_fwid:
...
#endif
    {
        ...
        if (ret) {
            ...
            ret = cts_fw_reg_readw_retry(cts_dev, CTS_DEVICE_FW_REG_VERSION,
                &device_fw_ver, 5, 0);
            if (ret) {
        ...
        void *gesture_info, bool trace_point)
{
    ...
    if (trace_point) {
        ret = cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_GESTURE_INFO + 2,
            gesture_info + 2, (((u8 *) gesture_info))[1] *
