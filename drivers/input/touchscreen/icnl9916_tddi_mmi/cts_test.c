...
static int get_test_result(struct cts_device *cts_dev, u16 *result)
{
    ...
    ret = cts_get_rawdata(cts_dev, (u8 *)result);
    if (ret) {
...
static int validate_test_result(struct cts_device *cts_dev,
        const char *desc, u16 *test_result, u16 min, u16 max)
{
#define SPLIT_LINE_STR \
    "------------------------------"
    ...
    for (r = 0; r < cts_dev->fwdata.rows; r++) {
        for (c = 0; c < cts_dev->fwdata.cols; c++) {
            if (test_result[r * cts_dev->fwdata.cols + c] < min ||
                ...
                cts_info("  %3d: [%-2d][%-2d] = %u", failed_cnt, r, c,
...
static int validate_compensate_cap(struct cts_device *cts_dev,
        const char *desc, u8 *test_result, u16 min, u16 max)
{
#define SPLIT_LINE_STR \
    "------------------------------"
        ...
        for (c = 0; c < cts_dev->fwdata.cols; c++) {
            if (test_result[r * cts_dev->fwdata.cols + c] < min ||
                ...
                cts_info("  %3d: [%-2d][%-2d] = %u", failed_cnt,
                     r, c, test_result[r * cts_dev->fwdata.cols + c]);
            }
        }
    }
...
unlock_device:
    ...
    if (data_valid) {
        ...
        ret = validate_test_result(cts_dev, "Rawdata", rawdata, min, max);
    }
    kfree(rawdata);
...
int cts_noise_test(struct cts_device *cts_dev, u32 frames, u16 max)
{
    ...
    while (frames--) {
        ...
        if (unlikely(first_frame)) {
            memcpy(max_rawdata, curr_rawdata, TEST_RESULT_BUFFER_SIZE(cts_dev));
            memcpy(min_rawdata, curr_rawdata, TEST_RESULT_BUFFER_SIZE(cts_dev));
            first_frame = false;

            continue;
        } else {
            for (i = 0; i < cts_dev->fwdata.rows * cts_dev->fwdata.cols; i++) {
...
unlock_device:
    ...
    if (data_valid) {
        ...
        ret = validate_test_result(cts_dev, "Noise test", noise, 0, max);
    }
