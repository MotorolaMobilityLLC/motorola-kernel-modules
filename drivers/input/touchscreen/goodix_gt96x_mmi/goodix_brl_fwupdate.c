		...
		struct firmware_data *fw_data)
{
	...
	int subsys_info_offset = cd->update_ctrl.update_info->subsys_info_offset;
 ...
 *		   < 0 failed
 */
static int goodix_reg_write_confirm(struct goodix_ts_core *cd, unsigned int addr,
				...
				const char *name)
{
	...
	if (retry < 0) {
		ts_info("can't find Firmware image [%s]", name);
		return r;
	}
...
int goodix_fw_update_init(struct goodix_ts_core *core_data)
{
	...
	core_data->update_ctrl.initialized = true;
	...
	ts_info("success init fw update");
	return 0;
}
