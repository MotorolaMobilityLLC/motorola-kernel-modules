...
#define DBG(fmt, ...) \
	pr_info("AWINIC_BIN %s, line = %d " fmt, __func__, __LINE__, ##__VA_ARGS__)

#define DBG_ERR(fmt, ...) \
	pr_err("AWINIC_BIN_ERR %s, line = %d " fmt, __func__, __LINE__, ##__VA_ARGS__)
#else
#define DBG(fmt, ...)
#define DBG_ERR(fmt, ...)
#endif
 ...
 *
 */
static int aw8680x_check_sum(struct aw_bin *bin, int bin_num)
{
	unsigned int i = 0;
...
static int aw8680x_check_data_version(struct aw_bin *bin, int bin_num)
{
	int i = 0;
...
static int aw8680x_check_register_num_v1(struct aw_bin *bin, int bin_num)
{
...
static int aw8680x_check_dsp_reg_num_v1(struct aw_bin *bin, int bin_num)
{
...
static int aw8680x_check_soc_app_num_v1(struct aw_bin *bin, int bin_num)
{
 ...
 *
 */
static void aw8680x_get_single_bin_header_1_0_0(struct aw_bin *bin)
{
	int i;
...
static int aw8680x_parse_each_of_multi_bins_1_0_0(unsigned int bin_num, int bin_serial_num,
				      struct aw_bin *bin)
{
	int ret = 0;
	...
	ret = aw8680x_parse_bin_header_1_0_0(bin);
	return ret;
}
 ...
 */
static int aw8680x_get_multi_bin_header_1_0_0(struct aw_bin *bin)
{
	int i = 0;
	int ret = 0;
	...
	aw8680x_get_single_bin_header_1_0_0(bin);

	for (i = 0; i < bin_num; i++) {
		DBG("aw_bin_parse enter multi bin for is %d\n", i);
		ret = aw8680x_parse_each_of_multi_bins_1_0_0(bin_num, i, bin);
		if (ret < 0)
			return ret;
	}
	return 0;
 ...
 *
 */
int aw8680x_parse_bin_header_1_0_0(struct aw_bin *bin)
{
	int ret = 0;
	...
	case DATA_TYPE_SOC_APP:
		...
		DBG("bin->single_bin_parse_num is %d\n", bin->single_bin_parse_num);
	...
	case DATA_TYPE_MULTI_BINS:
		...
		DBG("bin->multi_bin_parse_num is %d\n", bin->multi_bin_parse_num);
		ret = aw8680x_get_multi_bin_header_1_0_0(bin);
		if (ret < 0)
			return ret;
		break;
	default:
...
static int aw8680x_check_bin_header_version(struct aw_bin *bin)
{
	int ret = 0;
...
int aw8680x_parsing_bin_file(struct aw_bin *bin)
{
	int i = 0;
	int ret = 0;
	...
	ret = aw8680x_check_bin_header_version(bin);
	if (ret < 0) {
	...
	for (i = 0; i < bin->all_bin_parse_num; i++) {
		/* check sum */
		ret = aw8680x_check_sum(bin, i);
		if (ret < 0) {
			DBG_ERR("aw_bin_parse check sum data error\n");
			return ret;
		}
		/* check bin data version */
		ret = aw8680x_check_data_version(bin, i);
		if (ret < 0) {
		...
		if (bin->header_info[i].bin_data_ver == DATA_VERSION_V1) {
			...
			if (bin->header_info[i].bin_data_type ==
			    DATA_TYPE_REGISTER) {
				ret = aw8680x_check_register_num_v1(bin, i);
				if (ret < 0) {
				...
				/* check dsp reg num */
				   ...
				   DATA_TYPE_DSP_REG) {
				ret = aw8680x_check_dsp_reg_num_v1(bin, i);
				if (ret < 0) {
				...
				/* check soc app num */
				   ...
				   DATA_TYPE_SOC_APP) {
				ret = aw8680x_check_soc_app_num_v1(bin, i);
				if (ret < 0) {
