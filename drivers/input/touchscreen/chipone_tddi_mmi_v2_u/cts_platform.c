...
int cts_plat_spi_setup(struct cts_platform_data *pdata)
{
    ...
    // cts_info("chip_select  :%d", pdata->spi_client->chip_select[0]);
    ...
    */
    ...
    if (contact) {
        if (delayed_work_pending(&pdata->touch_event_timeout_work)) {
            mod_delayed_work(g_cts_data->workqueue,
                    &pdata->touch_event_timeout_work, msecs_to_jiffies(100));
        } else {
            queue_delayed_work(g_cts_data->workqueue,
                    &pdata->touch_event_timeout_work, msecs_to_jiffies(100));
        }
    } else {
    ...
    if (contact) {
        if (delayed_work_pending(&g_cts_data->heart_work)) {
            mod_delayed_work(g_cts_data->heart_workqueue,
                    &g_cts_data->heart_work, msecs_to_jiffies(2000));
        } else {
            queue_delayed_work(g_cts_data->heart_workqueue,
                    &g_cts_data->heart_work, msecs_to_jiffies(2000));
        }
    }
#endif
	...
	if (!contact) {
		...
		if (touch_down) {
		    g_cts_data->last_event_time = ktime_get_boottime();
...
#ifdef CHIPONE_SENSOR_EN
    PM_WAKEUP_REGISTER(g_cts_data->device, gesture_wakelock, "poll-wake-lock");
    if (!gesture_wakelock) {
