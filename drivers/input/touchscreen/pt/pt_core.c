...
static void pt_restart_wd_timer(struct pt_core_data *cd)
{
	if (cd->watchdog_interval < 100) {
		pt_debug(cd->dev, DL_ERROR,
			"%s: WARNING: Invalid watchdog interval: %d\n",
	...
	mod_timer(&cd->watchdog_timer, jiffies +
		msecs_to_jiffies(PT_WATCHDOG_RESTART_TIMEOUT));
	cd->watchdog_enabled = 1;
	pt_debug(cd->dev, DL_INFO, "%s: TTDL WD Restarted\n", __func__);
}
...
irqreturn_t pt_irq(int irq, void *handle)
{
	...
	spin_lock_irqsave(&cd->int_spinlock, flags);
	if (cd->int_cnt < 0xFFFFFFFF) {
		cd->int_cnt++;
	}
	spin_unlock_irqrestore(&cd->int_spinlock, flags);
...
static int pt_core_wake_device_from_deep_standby_(struct pt_core_data *cd)
{
	...
	/* Wait Exit Bootloader Mode */
	rc = _pt_poll_for_fw_exit_boot_mode(cd, 400, &wait_time);
	pt_debug(cd->dev, DL_INFO, "%s: wait change scanmode %dms\n",
		__func__, wait_time);
...
static int pt_recovery_hw(struct pt_core_data *cd)
{
	int rc = 0;
	bool old_irq_enabled;

	/* Power OFF/ON */
	if (cd->cpdata->setup_power) {
		cd->hw_detected = false;
		cd->startup_status = STARTUP_STATUS_START;
		old_irq_enabled = cd->irq_enabled;
		if (old_irq_enabled) {
			disable_irq(cd->irq);
			cd->irq_enabled = false;
		}
		cd->cpdata->setup_power(cd->cpdata, PT_MT_POWER_OFF, cd->dev);
		mdelay(20);
		cd->cpdata->setup_power(cd->cpdata, PT_MT_POWER_ON, cd->dev);
		if (old_irq_enabled) {
			enable_irq(cd->irq);
			cd->irq_enabled = true;
		}
	...
	if (!rc) {
		*enum_status |= STARTUP_STATUS_FW_OUT_OF_BOOT;
		pt_debug(cd->dev, DL_WARN,
			...
			__func__, wait_time);
		{
			u8 after_mode = PT_MODE_UNKNOWN;
			if (!pt_pip2_get_mode_sysmode_(cd, &after_mode, NULL)) {
				pt_debug(cd->dev, DL_WARN,
					"%s: Exit BOOT Mode Current Mode:%d\n",
					__func__, after_mode);
				cd->mode = after_mode;
			}
		}
...
static int pt_core_rt_suspend(struct device *dev)
{
	...
	rc = pt_core_sleep(cd);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: Error on sleep - wakeup recovery\n", __func__);
	}
	...
	cd->int_cnt = 0;
	spin_unlock_irqrestore(&cd->int_spinlock, flags);
	pt_debug(dev, DL_WARN, "%s: EnterMode:%d GLV:%d REP:%d IntCount:0x%x FW:0x%x CFG:0x%x\n",
							__func__, cd->easy_wakeup_gesture, cd->glove_mode, cd->report_mode, int_cnt, ttdata->revctrl, ttdata->fw_ver_conf);
	return 0;
}
...
static int pt_core_rt_resume(struct device *dev)
{
	...
	rc = pt_core_wake(cd);
	if (rc < 0) {
		pt_debug(cd->dev, DL_WARN, "%s: Start HW recovery\n", __func__);
		pt_stop_wd_timer(cd);
		mutex_lock(&cd->system_lock);
		cd->sysinfo.ready = false;
		mutex_unlock(&cd->system_lock);
	...
	if (!cd->sysinfo.ready) {
		/* If the system information does not exist, it will be reacquired. */
		pt_debug(cd->dev, DL_WARN, "%s: Start Recovery Enumeration\n", __func__);
		pt_queue_enum(cd);
	}
...
static int pt_core_suspend(struct device *dev)
{
	...
	if(!strncmp(client->name, PT_I2C_NAME, strlen(PT_I2C_NAME))){
		...
		unsigned long event, void *data)
{
	...
	if (event != DRM_PANEL_EVENT_BLANK || !evdata)
		goto exit;

	blank = evdata->data;
	if (*blank == DRM_PANEL_EVENT_UNBLANK) {
		pt_debug(cd->dev, DL_WARN, "%s: UNBLANK!\n", __func__);
		if (cd->fb_state != FB_ON) {
	...
	} else if (*blank == DRM_PANEL_EVENT_BLANK_LP) {
		pt_debug(cd->dev, DL_WARN, "%s: BLANKLP!\n", __func__);
		if (cd->fb_state != FB_ON) {
			call_atten_cb(cd, PT_ATTEN_RESUME, 0);
			cd->fb_state = FB_ON;
		}
	} else if (*blank == DRM_PANEL_BLANK_POWERDOWN) {
		pt_debug(cd->dev, DL_WARN, "%s: POWERDOWN!\n", __func__);
		if (cd->fb_state != FB_OFF) {
#ifdef CYPSOC_PICOLEAF_ENABLE
...
static void pt_watchdog_work(struct work_struct *work)
{
	...
	/*
	 * if found the current sleep_state is SS_SLEEPING/SS_WAKING
	 * then no need to request_exclusive, directly return
	 */
	if ((cd->sleep_state == SS_SLEEPING) || (cd->sleep_state == SS_WAKING))
		return;
...
queue_startup:
	if (rc) {
		...
		if (cd->startup_retry_count++ < PT_WATCHDOG_HWRESET_RETRY_COUNT) {
			/*
...
/* GLOVE MODE */
#define PT_TOUCHMODE_GLOVE		0x02
#define PT_GLOVE_MODE_DISABLE	0
#define PT_GLOVE_MODE_ENABLE	1

static ssize_t pt_touch_glove_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	u32 input_data[2], mode;
	int length;
	int rc = 0;
	...
	if (input_data[0] != PT_GLOVE_MODE_ENABLE && input_data[0] != PT_GLOVE_MODE_DISABLE) {
		dev_err(dev, "%s: Invalid param\n", __func__);
		rc = -EINVAL;
		goto exit;
	}
	...
	mutex_lock(&cd->system_lock);
	cd->glove_mode = (u8)input_data[0];
	mutex_unlock(&cd->system_lock);
	rc = pt_pip_get_param(cd, PT_RAM_ID_TOUCHMODE_ENABLED, &mode);
	if (rc) {
		pt_debug(cd->dev, DL_ERROR,
				"%s: fail to read param\n", __func__);
		goto fail_rw;
	}
	mode = (cd->glove_mode) ? mode | PT_TOUCHMODE_GLOVE : mode & ~PT_TOUCHMODE_GLOVE;
	rc = pt_pip_set_param(cd, PT_RAM_ID_TOUCHMODE_ENABLED,
						  mode, PT_RAM_ID_TOUCHMODE_ENABLED_SIZE);
	if (rc) {
		pt_debug(cd->dev, DL_ERROR,
				"%s: fail to write param\n", __func__);
		goto fail_rw;
	}
	pt_debug(cd->dev, DL_WARN,
			"%s: Updated glove_mode = 0x%02X\n", __func__, cd->glove_mode);
fail_rw:
...
static ssize_t pt_touch_glove_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t count;
	...
	mutex_lock(&cd->system_lock);
	count = snprintf(buf, PT_MAX_PRBUF_SIZE, "%d\n", cd->glove_mode);
	mutex_unlock(&cd->system_lock);
	return count;
}

/* REPORT MODE */
#define PT_REPORT_MODE			0x01
#define PT_REPORT_MODE_NORMAL	0
#define PT_REPORT_MODE_ALWAYS	1

static ssize_t pt_touch_report_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	u32 input_data[2], mode;
	...
	if (input_data[0] != PT_REPORT_MODE_NORMAL && input_data[0] != PT_REPORT_MODE_ALWAYS) {
		dev_err(dev, "%s: Invalid param\n", __func__);
		rc = -EINVAL;
		goto exit;
	}
	...
	pm_runtime_get_sync(cd->dev);
	mutex_lock(&cd->system_lock);
	cd->report_mode = (u8)input_data[0];
	mutex_unlock(&cd->system_lock);
	rc = pt_pip_get_param(cd, PT_RAM_ID_ACT_DIST_ALWAYS_REPORT, &mode);
	if (rc) {
		pt_debug(cd->dev, DL_ERROR,
				"%s: fail to read param\n", __func__);
		goto fail_rw;
	}
	mode = (cd->report_mode) ? mode | PT_REPORT_MODE : mode & ~PT_REPORT_MODE;
	rc = pt_pip_set_param(cd, PT_RAM_ID_ACT_DIST_ALWAYS_REPORT,
						  mode, PT_RAM_ID_ACT_DIST_ALWAYS_REPORT_SIZE);
	if (rc) {
		pt_debug(cd->dev, DL_ERROR,
				"%s: fail to write param\n", __func__);
		rc = 0;
	...
	pt_debug(cd->dev, DL_WARN,
			"%s: Updated report_mode = 0x%02X\n", __func__, cd->report_mode);
fail_rw:
	pm_runtime_put(cd->dev);
...
static ssize_t pt_touch_report_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t count;
...
static DEVICE_ATTR(glove_mode, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
					pt_touch_glove_mode_show, pt_touch_glove_mode_store);

static DEVICE_ATTR(report_mode, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP,
					pt_touch_report_mode_show, pt_touch_report_mode_store);

static struct attribute *attributes_touch[] = {
	&dev_attr_fw_version.attr,
	&dev_attr_command.attr,
	&dev_attr_response.attr,
	&dev_attr_drv_debug.attr,
...
static struct attribute_group touch_attr_group = {
	.attrs = attributes_touch,
};

static int pt_add_touch_interfaces(struct pt_core_data *cd)
{
	int rc;
	struct device *dev = cd->dev;

	cd->touch_class = class_create(THIS_MODULE, "touchscreen");
	if (IS_ERR(cd->touch_class)) {
		dev_err(dev, "%s: Failed to create touchscreen class.\n", __func__);
		return -ENODEV;
	}

	cd->touch_dev = device_create(cd->touch_class, NULL, 0, cd, "touch");
	if (IS_ERR(cd->touch_dev)) {
		dev_err(dev, "%s: Failed to crate touch device.\n", __func__);
		rc = -ENODEV;
		goto err_device_create;
	}

	rc = sysfs_create_group(&cd->touch_dev->kobj, &touch_attr_group);
	if (rc < 0) {
		dev_err(dev, "%s: fail - sysfs_create_group\n", __func__);
		goto err_add_interfaces;
	}
	dev_dbg(dev, "%s: Success to add touchscreen interfaces.\n", __func__);

	return 0;

err_add_interfaces:
	device_destroy(cd->touch_class, 0);
	cd->touch_dev = NULL;
err_device_create:
	class_destroy(cd->touch_class);
	cd->touch_class = NULL;
	return rc;
}

static int pt_remove_touch_interfaces(struct pt_core_data *cd)
{
	if (!cd->touch_class) {
		return 0;
	}

	sysfs_remove_group(&cd->touch_dev->kobj, &touch_attr_group);
	device_destroy(cd->touch_class, 0);
	cd->touch_dev = NULL;
	class_destroy(cd->touch_class);
	cd->touch_class = NULL;
...
static int add_sysfs_interfaces(struct device *dev)
{
	...
	/* Add OEM Interface */
	ret = pt_add_touch_interfaces(cd);
	if (ret != 0){
		goto undo;
	}
	return 0;
...
/*******************************************************************************
 * FUNCTION: cypsoc_picoleaf_force_power_on_hw
 *
 * SUMMARY: Turn on Cypress PSoC for Picoleaf
 *
 * RETURN:
 *	 0 = success
 *	!0 = failure
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	spin_lock_init(&cd->int_spinlock);
	...
	/* Suspend PanelScan */
	rc = pt_pip_suspend_scanning_(cd);
	pt_debug(dev, DL_WARN, "%s: Scan Suspend rc=%d\n", __func__, rc);
	if (rc) {
		rc = pt_pip_suspend_scanning_(cd);
		pt_debug(dev, DL_WARN, "%s: Scan Suspend Retry rc=%d\n", __func__, rc);
	}
...

	{
		...
		for (i = 0; i < 3; i++) {
			...
			if ((pip_ver_major == 1) || (pip_ver_major == 2)) {
				pt_debug(dev, DL_WARN, "%s: Get PIP Version Success retry:%d\n", __func__, i);
				break;
			}
			msleep(20);
		}
...
skip_enum:
	...
	{
		int wait_time;
		_pt_poll_for_fw_exit_boot_mode(cd, 400, &wait_time);
	}
	pt_pip_resume_scanning_(cd);
...
#ifdef CYPSOC_PICOLEAF_ENABLE
	pt_debug(dev, DL_INFO, "%s: invokes cypress PSoC i2c ready function.\n",
			__func__);
...
#ifdef CYPSOC_PICOLEAF_ENABLE
		cypsoc_picoleaf_i2c_power_turned_off(cd->cypsoc_picoleaf_data);
#endif
	if (cd->cpdata->setup_power)
...
#elif defined(CONFIG_DRM)
	if (pt_active_panel)
		drm_panel_notifier_unregister(pt_active_panel, &cd->fb_notifier);
#endif
