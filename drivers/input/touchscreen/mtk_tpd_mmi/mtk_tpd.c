...
EXPORT_SYMBOL(tpd_get_gpio_info);
...
/* tpd driver init */
static int tpd_driver_init(struct platform_device *pdev)
{
...
{
	int i;
	struct platform_device *pdev = to_platform_device(tpd->tpd_dev);
