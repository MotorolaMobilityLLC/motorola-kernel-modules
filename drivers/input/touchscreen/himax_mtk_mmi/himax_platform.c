...
int himax_check_dt() {
	int ret = 0;
        ...
        I("enter\n");
        node1 = of_find_matching_node(node1, himax_match_table);
        if (node1) {
                int ret;
		ret = of_property_read_string(node1, "himax,panel-supplier", &tpd_device_driver.tpd_panel_supplier);
		I("get himax,panel_supplier ret=%d, tpd_panel_supplier= %s\n", ret, tpd_device_driver.tpd_panel_supplier);
        } else {
                I("node not found!\n");
        }
