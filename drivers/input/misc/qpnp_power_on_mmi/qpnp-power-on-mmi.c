...
static int qpnp_pon_mmi_probe(struct platform_device *pdev)
{
	...
	rc = of_property_read_string(pdev->dev.of_node, "interrupt-names", &irq_name);
	if (rc < 0 ) {
		dev_err(&pon->pdev->dev,
			"Can't get IRQ name %d\n",
			rc);
		return rc;
	}

	pon->kpd_bark_irq = platform_get_irq_byname(pon->pdev, irq_name);
	...
	dev_info(&pon->pdev->dev, "Request %d %s  IRQ\n",
		pon->kpd_bark_irq, irq_name);
