...
{
	seq_printf(file,"[Vendor]GF3658,A-KERR\n");
	return 0;
}

static int proc_open(struct inode *inode,struct file *file)
{
	printk("Goodixfp proc_open\n");
...
#endif
{
	...
	gf_dev->vdd = regulator_get_optional(&pdev->dev, "vdd");
	if (IS_ERR(gf_dev->vdd) || gf_dev->vdd == NULL) {
		gf_dev->vdd = NULL;
		pr_err("[Simon]%s get vdd-supply failed.\n", __func__);
	}
	...
	if (NULL == proc_entry)
	{
		printk("Goodix Couldn't create proc entry!");
		return -ENOMEM;
	}
	else
	{
		printk("Goodix Create proc entry success!");
	}
...
static int __init gf_init(void)
{
	...
	status = register_chrdev(SPIDEV_MAJOR, CHRD_DRIVER_NAME, &gf_fops);
        pr_warn("jianl: gf_init register return status=%d!\n", status);
	if (status < 0) {
	...
	if (IS_ERR(gf_class)) {
        ...
        pr_warn("jianl: gf_init successfully class_create: %s!\n", CLASS_NAME);
