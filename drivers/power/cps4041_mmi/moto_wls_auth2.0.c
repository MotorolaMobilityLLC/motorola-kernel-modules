...
int moto_wls_auth2_ops_register(struct wls_ops *ops)
{
	if (!ops) {
	...
	if (motoauth)
		...
		pr_err("%s motoauth is null\n", __func__);
		return -1;
	}

	return 0;
}
EXPORT_SYMBOL(moto_wls_auth2_ops_register);
