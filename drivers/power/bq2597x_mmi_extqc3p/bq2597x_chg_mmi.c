				    ...
				    u8 mask, u8 data)
{
	...
	if (bq->skip_reads || bq->skip_writes){
		bq_err("Skip read or write: skip_reads=%d, skip_writes= %d\n", bq->skip_reads, bq->skip_writes);
		return 0;
	}
...
static int bq2597x_enable_charge(struct bq2597x *bq, bool enable)
{
	...
	bq_err("bq2597x_enable_charge: enable: %d\n", enable);
	if (enable)
	...
	if (ret == 0) {
		...
		if (data == SC8551_DEVICE_ID || data == SC8551A_DEVICE_ID)
			...
			bq->chip_vendor = BQ25970;
                ...
                bq_info("detect device PN:%x, ID:%x, REV:%x, Chip:%x \n!",
                                bq->part_no, bq->device_id, bq->revision, bq->chip_vendor);
...
void bq2597x_set_psy(void)
{
	power_supply_changed(g_bq->usb_psy);
}
EXPORT_SYMBOL_GPL(bq2597x_set_psy);
