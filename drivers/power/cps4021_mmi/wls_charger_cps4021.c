...
typedef enum
{
	...
	CPS_COMM_REG_CMD,
	CPS_COMM_REG_FUNC_EN,
	CPS_COMM_REG_INFO_FLAG,
	CPS_COMM_REG_MAX
}cps_comm_reg_e;
...
cps_reg_s cps_comm_reg[CPS_COMM_REG_MAX] = {
    ...
    {CPS_COMM_REG_FUNC_EN,         4,              0x0030},
    {CPS_COMM_REG_INFO_FLAG,       4,              0x0034},
};

cps_reg_s cps_rx_reg[CPS_RX_REG_MAX] = {
    /* reg name            bytes number      reg address          */
    {CPS_RX_REG_EPT_VAL,         2,              0x011A},
    {CPS_RX_REG_VOUT_SET,        2,              0x0090},
    {CPS_RX_REG_OCP_TH,          2,              0x00B2},
    ...
    {CPS_RX_REG_RP_VAL,          3,              0x0118},
    {CPS_RX_REG_FOP_VAL,         2,              0x010E},
    {CPS_RX_REG_NEGO_DONE_POWER, 1,              0x011E},
...
cps_reg_s cps_tx_reg[CPS_TX_REG_MAX] = {
    /* reg name            bytes number      reg address          */
    {CPS_TX_REG_PPP_HEADER,      1,              0x0050},
    {CPS_TX_REG_PPP_COMMAND,     1,              0x0051},
    {CPS_TX_REG_PPP_DATA0,       1,               0x0052},
    {CPS_TX_REG_PPP_DATA1,       1,               0x0053},
    {CPS_TX_REG_PPP_DATA2,       1,               0x0054},
    {CPS_TX_REG_PPP_DATA3,       1,               0x0055},
    {CPS_TX_REG_PPP_DATA4,       1,               0x0056},
    {CPS_TX_REG_PPP_DATA5,       1,               0x0057},
    {CPS_TX_REG_PPP_DATA6,       1,               0x0058},
    {CPS_TX_REG_BC_HEADER,       1,              0x0070},
    {CPS_TX_REG_BC_COMMAND,      1,               0x0071},
    {CPS_TX_REG_BC_DATA0,        1,               0x0072},
    {CPS_TX_REG_BC_DATA1,        1,               0x0073},
    {CPS_TX_REG_BC_DATA2,        1,               0x0074},
    {CPS_TX_REG_BC_DATA3,        1,               0x0075},
    {CPS_TX_REG_BC_DATA4,        1,               0x0076},
    {CPS_TX_REG_BC_DATA5,        1,               0x0077},
    {CPS_TX_REG_BC_DATA6,        1,              0x0078},
...
{
    cps_reg_s *cps_reg;
    cps_reg = (cps_reg_s*)(&cps_rx_reg[CPS_RX_REG_NEGO_DONE_POWER]);
    return cps_wls_read_reg(cps_reg->reg_addr, (int)cps_reg->reg_bytes_len);
}
...
{
    cps_reg_s *cps_reg;
    cps_reg = (cps_reg_s*)(&cps_tx_reg[CPS_TX_REG_ADC_VPA]);
    return cps_wls_read_reg((int)cps_reg->reg_addr, (int)cps_reg->reg_bytes_len);
}
...
static int cps_wls_rx_irq_handler(int int_flag)
{
	...
	if (int_flag & RX_INT_DEBUG_INFO)
	{
		int temp;
        cps_reg_s *cps_reg;
        cps_reg = (cps_reg_s*)(&cps_comm_reg[CPS_COMM_REG_INFO_FLAG]);
        temp = cps_wls_read_reg((int)cps_reg->reg_addr, (int)cps_reg->reg_bytes_len);
        if(temp & RX_INFO_FLAG_MLDO_OFF)
        {
  		    cps_wls_log(CPS_LOG_DEBG, " CPS_WLS IRQ:  RX_INT_LDO_OFF");
            temp &= ~RX_INFO_FLAG_MLDO_OFF;
  	        cps_wls_write_reg(cps_reg->reg_addr, temp, (int)cps_reg->reg_bytes_len);
  	    }
  	    if (temp & RX_INFO_FLAG_MLDO_ON)
  	    {
  		    chip->rx_ldo_on = true;
            chip->rx_start_ktime = ktime_get_boottime();
			...
			if(chip->limit_wls_power_support) {
				int soc = cps_get_bat_info(POWER_SUPPLY_PROP_CAPACITY);
				if(soc >= 0 && chip->limit_wls_power_soc >= soc) {
	    ...
	    if(temp & RX_INFO_FLAG_VRECT_OVP_TO)
  		{
  			cps_wls_log(CPS_LOG_DEBG, " CPS_WLS IRQ:  RX_INT_OVP_TO");
  			temp &= ~RX_INFO_FLAG_VRECT_OVP_TO;
  			cps_wls_write_reg(cps_reg->reg_addr, temp, (int)cps_reg->reg_bytes_len);
  		}
  		if(temp & RX_INFO_FLAG_MLDO_HOCP)
  		{
  			cps_wls_log(CPS_LOG_DEBG, " CPS_WLS IRQ:  RX_INT_MLDO_HOCP");
  			temp &= ~RX_INFO_FLAG_MLDO_HOCP;
  			cps_wls_write_reg(cps_reg->reg_addr, temp, (int)cps_reg->reg_bytes_len);
  		}
  		if(temp & RX_INFO_FLAG_SR_BRI_SW_FAIL)
  		{
  			cps_wls_log(CPS_LOG_DEBG, " CPS_WLS IRQ:  RX_INT_SR_BRI_SW_FAIL");
  			temp &= ~RX_INFO_FLAG_SR_BRI_SW_FAIL;
  			cps_wls_write_reg(cps_reg->reg_addr, temp, (int)cps_reg->reg_bytes_len);
  		}
  		if(temp & RX_INFO_FLAG_SR_BRI_SW_SUCC)
  		{
  			cps_wls_log(CPS_LOG_DEBG, " CPS_WLS IRQ:  RX_INT_SR_BRI_SW_SUCC");
  			temp &= ~RX_INFO_FLAG_SR_BRI_SW_SUCC;
  			cps_wls_write_reg(cps_reg->reg_addr, temp, (int)cps_reg->reg_bytes_len);
  		}
	}
	...
	if (int_flag & RX_INT_NEGO_DONE)
	{
		cps_wls_log(CPS_LOG_DEBG, " CPS_WLS IRQ:  RX_INT_NEGO_POWER_READY");
		wlc_power = cps_wls_get_rx_neg_power() / 10;
		chip->mcode = MOTO_TX_MCODE;//cps_wls_get_mcode();
...
static int cps_wls_set_tx_en_pin(char *str, bool en)
{
	...
	if (chg && gpio_is_valid(chg->wls_tx_en)) {
        ...
        cps_wls_log(CPS_LOG_DEBG, "[%s] cps_wls_get_tx_en_pin before: status:%d\n", str, status);
		gpio_set_value(chg->wls_tx_en, en);
		rt = CPS_WLS_SUCCESS;
	}

	cps_wls_log(CPS_LOG_DEBG, "[%s] cps_wls_set_tx_en_pin en:%d rt:%d\n", str, en, rt);
...
static int wireless_fw_update(bool force)
{
    ...
    cps_wls_set_tx_en_pin("wireless_fw_update", true);
...
free_bug:
	...
	//wireless_chip_reset();
	cps_wls_set_tx_en_pin("wireless_fw_update", false);
	return ret;
...
static int cps_wls_parse_dt(struct cps_wls_chrg_chip *chip)
{
   ...
   chip->wls_det_int = of_get_named_gpio(node, "cps_det_int", 0);
    ...
    chip->wls_tx_en = of_get_named_gpio(node, "wls_tx_en", 0);
    if(!gpio_is_valid(chip->wls_tx_en))
		cps_wls_log(CPS_LOG_ERR,"wls_tx_en is %d invalid\n", chip->wls_tx_en );
...
static int cps_wls_gpio_request(struct cps_wls_chrg_chip *chip)
{
	...
	if(gpio_is_valid(chip->wls_tx_en)) {
		ret  = devm_gpio_request_one(chip->dev, chip->wls_tx_en,
				  GPIOF_OUT_INIT_LOW, "wls_tx_en");
		if (ret  < 0)
			cps_wls_log(CPS_LOG_ERR," [%s] Failed to request wls_tx_en gpio, ret:%d", __func__, ret);
	}

	if(chip->enable_wlc_hall  != 0) {
		ret = devm_gpio_request_one(chip->dev, chip->wls_hal_int1,
					  GPIOF_IN, "cps4021_wls_hal_int1");
		if (ret < 0) {
...
static int wireless_get_chip_id(void *input)
{
    ...
    cps_wls_set_tx_en_pin("wireless_get_chip_id", true);
	chip->chip_id = cps_wls_get_chip_id();

	if (chip->chip_id != CPS_WLS_FAIL) {
		cps_wls_set_tx_en_pin("wireless_get_chip_id", false);
		return chip->chip_id;
	}
...
static int cps_wls_rx_power_on(void)
{
	...
	if(chip_id == 0x4021 && sys_mode == SYS_MODE_RX) {
		rx_power = true;
		rx_power_cnt = 0;
...
static int cps_wls_chrg_probe(struct i2c_client *client)
{
    ...
    chip->regmap32 = devm_regmap_init_i2c(client, &cps4021_regmap32_config);
    if (IS_ERR(chip->regmap)) {
        cps_wls_log(CPS_LOG_ERR, "[%s] Failed to allocate regmap!\n", __func__);
        devm_kfree(&client->dev, chip);
        return PTR_ERR(chip->regmap);
