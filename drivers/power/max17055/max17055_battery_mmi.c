...
#include <linux/power/max17055_battery_mmi.h>
#include <linux/of.h>
...
static int max17055_update_temp_config(struct max17055_chip *chip)
{
    ...
    struct regmap *map = chip->regmap;
    int ret = 0, temp, i;
    static unsigned int nUpdateFlag, nRcomp0, nTempCo, nRcompSeg;
    ...
    if (temp > 0) {
        if (!nUpdateFlag) {		/* Backup current values */
            ret |= regmap_read(map, MAX17055_RComp0, &nRcomp0);
            ret |= regmap_read(map, MAX17055_TempCo, &nTempCo);
            ret |= regmap_read(map, MAX17055_RcompSeg, &nRcompSeg);
            nRcompSeg = (nRcompSeg & 0xff) * 16;
        }
        else {		/* Restore backup values */
            ret |= max17055_write_verify_reg(map, MAX17055_RComp0, nRcomp0);
            ret |= max17055_write_verify_reg(map, MAX17055_TempCo, nTempCo);
            for (i = 0; i < 16; i++) {
                ret |= max17055_write_verify_reg(map, 0xA0+i, nRcompSeg);
            }
...
{
    struct max17055_chip *chip = power_supply_get_drvdata(psy);
    struct regmap *map = chip->regmap;
    int ret;
    u32 data;
...
max17055_get_pdata(struct device *dev)
{
    ...
    np = of_get_child_by_name(np, "maxim,configuration");
    if (!np)
        return dev->platform_data;;
    ...
    if (MAX17055_BATT_ID_ATL == max17055_get_battid()) {
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    for (reg = MAX17055_Status; reg <= MAX17055_VFSOC; reg++) {
        ...
        size = snprintf(buf, VALUE_MAX_LENGTH, "%04x = %04xh ", reg, data);
...
static const struct power_supply_desc max17055_no_current_sensepsy_desc = {
    .name		= "max17055_battery",
    .type		= POWER_SUPPLY_TYPE_MAIN,
    .get_property	= max17055_get_property,
    .set_property	= max17055_set_property,
    .property_is_writeable	= max17055_property_is_writeable,
    .properties	= max17055_battery_props,
    .num_properties	= ARRAY_SIZE(max17055_battery_props)- 2,
};

static const struct power_supply_desc max17055_psy_desc = {
    .name		= "max17055_battery",
    .type		= POWER_SUPPLY_TYPE_MAIN,
    .get_property	= max17055_get_property,
    .set_property	= max17055_set_property,
                          ...
                          const struct i2c_device_id *id)
{
    ...
    const struct power_supply_desc *max17055_desc = &max17055_psy_desc;
    ...
    if (!chip->pdata->enable_current_sense)
        max17055_desc = &max17055_no_current_sensepsy_desc;
    ...
    chip->battery = devm_power_supply_register(&client->dev, max17055_desc,
                    &psy_cfg);
