...
#define pr_fmt(fmt)     "GLINK_CHG:TA: %s: " fmt, __func__
...
int  trusted_shash_alloc(void)
{
	...
	if (IS_ERR(hmacalg)) {
		pr_err("trusted_key: could not allocate crypto %s\n",
			hmac_alg);
...
int trusted_sha1_set(const unsigned char *data,
				...
				unsigned char *digest)
{
	...
	if (IS_ERR(sdesc)) {
		pr_err("trusted_key: can't alloc %s\n", hash_alg);
...
int trusted_sha1(u32 *data, u32 data_size, unsigned char *digest)
{
	u8 num[4] = {0};
	u32 random_num[4] = {0};
	int i = 0, ret = 0;
