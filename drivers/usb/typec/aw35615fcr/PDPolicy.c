...
void USBPDEnable(Port_t *port, AW_BOOL DeviceUpdate, SourceOrSink TypeCDFP)
{
	...
	port->PortConfig.reqGetStatusAsSink = AW_FALSE;
...
void USBPDDisable(Port_t *port, AW_BOOL DeviceUpdate)
{
	...
	port->old_policy_state = peDisabled;
...
{
	if ((DPM_GetSourceCapHeader(port) == NULL) ||
			(DPM_GetSourceCap(port) == NULL))
		return;
	...
	case 0:
		if (PolicySendData(port,
				...
				DPM_GetSourceCap(port),
				DPM_GetSourceCapHeader(port)->
				NumDataObjects*4,
				peSourceSendCaps, 1,
...
void PolicySourceTransitionSupply(Port_t *port)
{
	...
	case 1:
		...
		newIsPPS = (DPM_GetSourceCap(port)
	...
	case 3:
		port->PolicyHasContract = AW_TRUE;
		DPM_TransitionSource(port, port->USBPDContract.FVRDO.ObjectPosition - 1);
	...
	case 4:
		...
		else if (DPM_IsSourceCapEnabled(port,
	...
	if (port->ProtocolMsgRx) {
		...
		if (port->PolicyRxHeader.NumDataObjects == 0) {
			...
			case CMTGetSinkCap:
				...
				break;
			case CMTDR_Swap:
				notify_observers(DATA_ROLE, port->PortID);
...
void PolicySourceGiveSinkCap(Port_t *port)
{
	...
	if ((DPM_GetSinkCapHeader(port) == NULL) ||
			(DPM_GetSinkCap(port) == NULL))
		return;

	port->old_policy_state = peSourceGiveSinkCaps;
#ifdef AW_HAVE_DRP
	if (port->PolicyRxHeader.SpecRevision == USBPDSPECREV2p0)
		NumDataObjects = DPM_GetSinkCapHeader(port)->NumDataObjects - port->snk_pps_size;
	else
		NumDataObjects = DPM_GetSinkCapHeader(port)->NumDataObjects;
...
void PolicySourceSendPRSwap(Port_t *port)
{
	...
	case 2:
		...
		if (TimerExpired(&port->PolicyStateTimer)) {
			platform_set_pps_voltage(port->PortID, SET_VOUT_0000MV);
			notify_observers(POWER_ROLE, port->PortID);
...
//				((DPM_GetSourceCap(port)[0].FPDOSupply.
...
void PolicySourceAlertReceived(Port_t *port)
{
	if (port->PolicyRxDataObj[0].ADO.Battery ||
		...
		/* Send Get_Status */
		notify_observers(ALERT_EVENT, port->PortID);
	}
...
void PolicySinkEvaluateCaps(Port_t *port)
{
	...
	for (i = DPM_GetSinkCapHeader(port)->NumDataObjects; i > 0; i--) {
		if (DPM_GetSinkCap(port)[i - 1].PDO.SupplyType == pdoTypeFixed) {
			for (j = 0; j < port->SrcCapsHeaderReceived.NumDataObjects; j++) {
				...
				if (port->SrcCapsReceived[j].PDO.SupplyType == pdoTypeFixed) {
					if (port->SrcCapsReceived[j].FPDOSupply.Voltage  == DPM_GetSinkCap(port)[i - 1].FPDOSink.Voltage) {
	...
	if ((port->src_support_fixed == AW_TRUE) || (src_support_pps == AW_TRUE)) {
		...
		if (src_support_pps == AW_TRUE) {
			port->SinkRequest.PPSRDO.OpCurrent = DPM_GetSinkCap(port)[i - 1].PPSAPDO.MaxCurrent;
			port->SinkRequest.PPSRDO.Voltage = DPM_GetSinkCap(port)[i - 1].PPSAPDO.MinVoltage * 5;
			port->PpsEnabled = AW_TRUE;
		} else if (port->src_support_fixed == AW_TRUE) {
			port->PpsEnabled = AW_FALSE;
			if (port->SrcCapsReceived[j].FPDOSupply.MaxCurrent >= DPM_GetSinkCap(port)[i - 1].FPDOSink.OperationalCurrent)
				port->SinkRequest.FVRDO.OpCurrent = DPM_GetSinkCap(port)[i - 1].FPDOSink.OperationalCurrent;
			else
		...
		if (port->ProtocolMsgRx) {
			...
			if (port->PolicyRxHeader.NumDataObjects == 0) {
				...
				case CMTAccept:
					...
					notify_observers(PD_NEW_CONTRACT, port->PortID);
					port->PpsEnabled =
...
void PolicySinkReady(Port_t *port)
{
	...
	else if (port->PortConfig.reqGetStatusAsSink) {
		SetPEState(port, peGetStatus);
		port->PortConfig.reqGetStatusAsSink = AW_FALSE;
	}
...
void PolicySinkGiveSinkCap(Port_t *port)
{
	...
	port->old_policy_state = peSinkGiveSinkCap;
	if (port->PolicyRxHeader.SpecRevision == USBPDSPECREV2p0)
		NumDataObjects = DPM_GetSinkCapHeader(port)->NumDataObjects - port->snk_pps_size;
	else
		NumDataObjects = DPM_GetSinkCapHeader(port)->NumDataObjects;
...
{
	AW_U32 sido = 0x80000000;
	AW_U32 power_src;

	power_src = port->src_pdo_vol[port->src_pdo_size - 1] * port->src_pdo_cur[port->src_pdo_size - 1] / 1000;
	sido = sido | (power_src / 1000);
	sido = sido | ((power_src / 1000) << 8);
	sido = sido | ((power_src / 1000) << 16);
...
void PolicySinkGiveSourceCap(Port_t *port)
{
	...
	if (port->PortConfig.PortType == USBTypeC_DRP) {
		PolicySendData(port, DMTSourceCapabilities,
					DPM_GetSourceCap(port),
					DPM_GetSourceCapHeader(port)->NumDataObjects *
...
void PolicyGetStatus(Port_t *port)
{
	if (port == NULL) {
		return;
	}
	switch (port->PolicySubIndex) {
	case 0:
		if (PolicySendCommand(port, CMTGetStatus, peGetStatus, 1,
					SOP_TYPE_SOP) == STAT_SUCCESS)
			TimerStart(&port->PolicyStateTimer, tSenderResponse);
		break;
	default:
		if (port->ProtocolMsgRx) {
			port->ProtocolMsgRx = AW_FALSE;
			port->internal_temp = 0;
			if ((port->PolicyRxHeader.NumDataObjects > 0) &&
					(port->PolicyRxHeader.MessageType == EXTStatus)) {
				SetPEState(port, port->PolicyIsSource ?
						peSourceReady : peSinkReady);
				port->internal_temp = port->ProtocolRxBuffer[7];
				AW_LOG("enter internal_temp=0x%x\n", port->ProtocolRxBuffer[7]);
			} else if (port->PolicyRxHeader.MessageType == CMTNotSupported ||
					port->PolicyRxHeader.MessageType == CMTReject) {
				SetPEState(port, port->PolicyIsSource ? peSourceReady : peSinkReady);
				AW_LOG("not support or reject status message\n");
			} else
				SetPEState(port, port->PolicyIsSource ?
...
void processDMTBIST(Port_t *port)
{
	...
	notify_observers(BIST_ENABLED, port->PortID);

	switch (bdo) {
	case BDO_BIST_Carrier_Mode_2:
		/* Only enter BIST for 5V contract */
		if (DPM_GetSourceCap(port)[
