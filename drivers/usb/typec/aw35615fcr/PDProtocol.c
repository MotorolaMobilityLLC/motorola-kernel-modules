...
void ProtocolSendingMessage(Port_t *port)
{
	...
	if (port->Registers.Status.I_TXSENT || port->Registers.Status.I_CRC_CHK) {
		...
		if (((port->old_policy_state == peSinkGiveSinkCap) || (port->old_policy_state == peSourceGiveSinkCaps)) &&
				((port->PolicyState == peSinkSendSoftReset) || (port->PolicyState == peSourceSendSoftReset))) {
			PolicySendHardReset(port);
			AW_LOG("SoftReset retry fail send hardreset\n");
		}
