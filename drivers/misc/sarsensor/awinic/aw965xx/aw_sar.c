...
#define AW_SAR_DRIVER_VERSION	"v0.7.0"
#if 0 //KERNEL_VERSION(4, 9, 0) <= LINUX_VERSION_CODE
...
int32_t aw_sar_parse_dts_comm(struct device *dev, struct device_node *np, struct aw_sar_dts_info *p_dts_info)
{
	...
	AWLOGI(dev, "aw_sar,monitor_esd = <%d>", p_dts_info->monitor_esd_flag);
...
static void aw_sar_input_free(struct aw_sar *p_sar)
{
	if ((p_sar->p_sar_para->p_platform_config != NULL) &&
		...
		AWLOGD(p_sar->dev, "aw_sar_input_free");
	}
}
...
static int32_t aw_sar_chip_effuse_status_check(struct aw_sar *p_sar)
{
	if (p_sar->p_sar_para->p_effuse_check != NULL)
		return p_sar->p_sar_para->p_effuse_check(p_sar);
...
int32_t aw_sar_update_fw(struct aw_sar *p_sar)
{
	...
	AWLOGD(p_sar->dev, "enter");
	p_sar->driver_update_coderam_reg = AW_SAR_CODERAM_UPLOADING;
	if (aw_sar_update_fw_para(p_sar, p_sar->p_sar_para->p_fw_bin) != AW_OK)
		return -AW_ERR;

	ret = aw_sar_load_bin_comm(p_sar);

	p_sar->driver_update_coderam_reg = AW_SAR_CODERAM_COMPELETE;
...
static int32_t aw_sar_awrw_data_analysis(struct aw_sar *p_sar, const char *buf, uint16_t len)
{
	uint32_t i = 0;
...
static int32_t aw_sar_awrw_handle(struct aw_sar *p_sar, const char *buf)
{
	...
	AWLOGD(p_sar->dev, "i2c_tranfar_data_len = %d, addr_len = %d, data_len = %d, reg_num=%d",
			p_sar->awrw_info.i2c_tranfar_data_len, p_sar->awrw_info.addr_len, p_sar->awrw_info.data_len, p_sar->awrw_info.reg_num);
	if (p_sar->awrw_info.p_i2c_tranfar_data != NULL) {
		devm_kfree(p_sar->dev, p_sar->awrw_info.p_i2c_tranfar_data);
		p_sar->awrw_info.p_i2c_tranfar_data = NULL;
	}
...
static int aw_sar_ps_notify_init(struct aw_sar *p_sar)
{
	...
	if (!psy) {
		AWLOGE(p_sar->dev, "Unable to get power_supply: %s", USB_POWER_SUPPLY_NAME);
...
int32_t aw_sar_chip_init(struct aw_sar *p_sar)
{
	...
	ret = aw_sar_chip_effuse_status_check(p_sar);
	if (ret != AW_OK) {
		AWLOGE(p_sar->dev, "effuse check error!");
		goto communication_fail;
	}
...
static void aw_sar_monitor_work(struct work_struct *aw_work)
{
	...
	AWLOGI(&p_sar->i2c->dev, "enter");

	if (p_sar->driver_update_coderam_reg == AW_SAR_CODERAM_UPLOADING) {
		AWLOGI(&p_sar->i2c->dev, "upgradeing");
		goto next_monitor;
	}

	ret = aw_sar_i2c_read(p_sar->i2c, 0xF800, &data);
	if (ret != AW_OK) {
		AWLOGE(p_sar->dev, "read 0x0000 err: %d", ret);
		goto next_monitor;
	}
	if ((((data >> 8) & 0x1 )== 0) && p_sar->driver_code_initover_flag && p_sar->dts_info.update_fw_flag == true) {
		AWLOGE(p_sar->dev, "aw_chip may hard reset");
		aw_sar_disable_irq(p_sar);
		aw_sar_soft_reset(p_sar);

		ret = aw_sar_check_init_over_irq(p_sar);
		if (ret != AW_OK) {
			ret = aw_sar_i2c_read(p_sar->i2c, 0xF800, &val);
			if (ret != AW_OK) {
				AWLOGE(p_sar->dev, "read init over irq error");
				return;
			}
			AWLOGE(p_sar->dev, "0xF800=0x%04X", val);
			if ((val >> 16) == 0) {
				AWLOGE(p_sar->dev, "check 0xF800 error");
				return;
			}
		}
		aw_sar_update_fw(p_sar);
		aw_sar_update_reg_set_func(p_sar);
		aw_sar_enable_irq(p_sar);

	} else if (p_sar->fault_flag == AW_SAR_UNHEALTHY && p_sar->driver_code_initover_flag) {
		AWLOGE(p_sar->dev, "aw_sar may soft reset");
		aw_sar_disable_irq(p_sar);
		p_sar->fault_flag = AW_SAR_HEALTHY;   
		aw_sar_soft_reset(p_sar);
		ret = aw_sar_check_init_over_irq(p_sar);
...
static int32_t aw_sar_monitor_esd_init(struct aw_sar *p_sar)
{
	...
	if (!p_sar->monitor_wq) {
		AWLOGE(&p_sar->i2c->dev, "aw_sar_workqueue error");
		return -AW_ERR;
	}
...
#endif
{
	...
	if (p_sar->dts_info.monitor_esd_flag) {
		...
		if (ret != AW_OK) {
			AWLOGE(&i2c->dev, "monitor_esd_init error!");
			goto err_esd_init;
		}
	}
