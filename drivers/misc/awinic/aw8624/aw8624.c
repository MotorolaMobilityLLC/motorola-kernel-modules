...
static int aw8624_ram_init(struct aw8624 *aw8624)
{
	if (aw8624->ram_update_delay) {
		...
		schedule_delayed_work(&aw8624->ram_work, msecs_to_jiffies(5000));
	} else {
		aw8624_ram_update(aw8624);
	}
	return 0;
			   ...
			   struct device_node *np)
{
	...
	val = of_property_read_u32(np, "real-i2c-addr", &aw8624->real_i2c_addr);
	if (val) {
		aw8624->dts_addr_real = false;
	} else {
	...
	if (val != 0)
		dev_info(dev, "%s vib_wavloop not found\n",__func__);
	...
	if (val != 0)
		dev_info(dev, "%s vib_td_brake not found\n",__func__);
...
static int aw8624_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(aw8624->irq_gpio)&&
		...
		ret = devm_request_threaded_irq(&i2c->dev,
						...
						NULL, aw8624_irq, IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
						"aw8624", aw8624);
		if (ret != 0) {
