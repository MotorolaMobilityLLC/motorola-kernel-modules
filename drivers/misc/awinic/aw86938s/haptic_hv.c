...
#define HAPTIC_HV_DRIVER_VERSION	"v1.11.0"
...
static void rtp_trim_lra_cali(struct aw_haptic *aw_haptic)
{
	...
	while (count) {
		...
		if (theory_time == real_time) {
			...
			aw_haptic->osc_cali_data = aw_haptic->efuse_cali_data;
			return;
...
#ifdef AW_ADD_BRAKE_WAVE
				if (ret == 1 && en_replace && aw_haptic->info.is_enabled_inter_brake) {
...
static void haptic_init(struct aw_haptic *aw_haptic)
{
	...
	aw_haptic->vbat_pro_params[0] = AW8693XS_VBAT_PRO1_BST_DEFAULT;
	aw_haptic->vbat_pro_params[1] = AW8693XS_VBAT_PRO1_BST_IPEAK_DEFAULT;
	aw_haptic->vbat_pro_params[2] = AW8693XS_VBAT_PRO2_BST_DEFAULT;
	aw_haptic->vbat_pro_params[3] = AW8693XS_VBAT_PRO2_BST_IPEAK_DEFAULT;
	aw_haptic->vbat_pro_params[4] = AW8693XS_PRO3_GAIN_1_2;
	aw_haptic->efuse_cali_data = 0;
	aw_haptic->activate_mode = aw_haptic->info.mode;
