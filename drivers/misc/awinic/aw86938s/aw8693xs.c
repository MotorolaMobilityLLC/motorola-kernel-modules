...
{
	if (flag) {
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSCTRL4,
					 AW8693XS_BIT_SYSCTRL4_EN_DLL_MASK,
					 AW8693XS_BIT_SYSCTRL4_EN_DLL_ON);
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_I2SCFG1,
			     ...
			     struct snd_soc_dai *dai)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_I2SCFG2, mask, reg_val);
...
static int aw8693xs_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_GLBRD5, &reg_val, AW_I2C_BYTE_ONE);
		if (reg_val != 0x0a) {
...
static int aw8693xs_codec_probe(aw_snd_soc_codec_t *codec)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_I2SCFG1, reg_val, AW_I2C_BYTE_TWO);
...
static int aw8693xs_snd_soc_init(struct device *dev)
{
	...
	memcpy(dai, aw8693xs_dai, sizeof(aw8693xs_dai));
	...
	if (ret < 0) {
		aw_err("failed to register aw8693xs: %d", ret);
		return ret;
	}
...
static void aw8693xs_reg_unlock(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TMCFG, &reg_val, AW_I2C_BYTE_ONE);
	} else {
		/* Lock register */
...
static int aw8693xs_check_qualify(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_EFCFG6, &reg_val, AW_I2C_BYTE_ONE);
	if (!(reg_val & 0x80)) {
...
{
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_PLAYCFG2, &gain, AW_I2C_BYTE_ONE);
}
...
static void aw8693xs_set_bst_vol(struct aw_haptic *aw_haptic, uint32_t bst_vol)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_PLAYCFG1,
...
{
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_WAVCFG1 + wav, &seq, AW_I2C_BYTE_ONE);
}
...
static void aw8693xs_set_wav_loop(struct aw_haptic *aw_haptic, uint8_t wav, uint8_t loop)
{
	if (wav % 2) {
		loop <<= 0;
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_WAVCFG9 + (wav / 2),
...
{
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_RTPDATA, data, len);
}

static void aw8693xs_set_rtp_aei(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSINTM,
...
static void aw8693xs_set_ram_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_RAMADDRH, ram_addr, AW_I2C_BYTE_TWO);
}
...
static void aw8693xs_set_base_addr(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_RTPCFG1,
				 AW8693XS_BIT_RTPCFG1_BASE_ADDR_H_MASK, rtp_addr[0]);
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_RTPCFG2, &rtp_addr[1], AW_I2C_BYTE_ONE);
}

static void aw8693xs_auto_brake_mode(struct aw_haptic *aw_haptic, bool flag)
{
	if (flag) {
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_PLAYCFG3,
...
{
	if (flag) {
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_CONTCFG1,
...
static uint8_t aw8693xs_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_GLBRD5, &state, AW_I2C_BYTE_ONE);
...
static void aw8693xs_play_go(struct aw_haptic *aw_haptic, bool flag)
{
	...
	if (flag) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_PLAYCFG4, &reg_val, AW_I2C_BYTE_ONE);
	} else {
...
static void aw8693xs_vbat_ref_config(struct aw_haptic *aw_haptic, uint16_t vbat_ref)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG1,
...
static void aw8693xs_play_mode(struct aw_haptic *aw_haptic, uint8_t play_mode)
{
	...
	case AW_STANDBY_MODE:
		...
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSCTRL3,
...
static void aw8693xs_upload_lra(struct aw_haptic *aw_haptic, uint32_t flag)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRIMCFG3, &reg_val[1], AW_I2C_BYTE_ONE);
	switch (flag) {
	case AW_WRITE_ZERO:
		aw_info("write zero to trim_lra!");
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRIMCFG3, &reg_val[0], AW_I2C_BYTE_ONE);
		break;
	case AW_F0_CALI_LRA:
		aw_info("write f0_cali_data to trim_lra = 0x%02X", aw_haptic->f0_cali_data);
		cali_data = aw_haptic->f0_cali_data;
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRIMCFG4, &cali_data, AW_I2C_BYTE_ONE);
		break;
...
static uint8_t aw8693xs_get_trim_lra(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_TRIMCFG4, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw8693xs_protect_config(struct aw_haptic *aw_haptic, uint8_t prtime, uint8_t prlvl)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_PWMCFG1,
	...
	if (prlvl != 0) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_PWMCFG4, &prtime, AW_I2C_BYTE_ONE);
	} else {
		/* Disable */
		aw_info("disable protection mode");
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_PWMCFG3,
...
static void aw8693xs_cont_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_CONTCFG6,
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG7,
			     &aw_haptic->info.cont_drv2_lvl, AW_I2C_BYTE_ONE);
	/* haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG8, &drv1_time, AW_I2C_BYTE_ONE); */
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG9, &drv2_time, AW_I2C_BYTE_ONE);
...
static void aw8693xs_one_wire_init(struct aw_haptic *aw_haptic)
{
	...
	reg_val = 0x6C;
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_GLBCFG4, &reg_val, AW_I2C_BYTE_ONE);

	reg_val = AW8693XS_BIT_START_DLY_2P5MS;
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_GLBCFG2, &reg_val, AW_I2C_BYTE_ONE);
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_TRGCFG8,
...
static void aw8693xs_trig1_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_TRGCFG7,
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRGCFG1, &trig_config, AW_I2C_BYTE_ONE);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRGCFG4, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8693xs_trig2_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRGCFG2, &trig_config, AW_I2C_BYTE_ONE);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRGCFG5, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8693xs_trig3_param_config(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRGCFG3, &trig_config, AW_I2C_BYTE_ONE);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_TRGCFG6, &trig_config, AW_I2C_BYTE_ONE);
}
...
static void aw8693xs_interrupt_setup(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_SYSINT, &reg_val, AW_I2C_BYTE_ONE);
...
static void aw8693xs_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_RAMDATA, buf + i, size);
...
static void aw8693xs_get_first_wave_addr(struct aw_haptic *aw_haptic, uint8_t *wave_addr)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_RAMDATA, reg_val, AW_I2C_BYTE_THREE);
...
static void aw8693xs_get_wav_seq(struct aw_haptic *aw_haptic, uint32_t len)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_WAVCFG1, reg_val, len);
	for (i = 0; i < len; i++)
...
static void aw8693xs_get_bemf_peak(struct aw_haptic *aw_haptic, uint16_t *peak)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_CONTRD18, reg_val, 4);
	peak[0] = (reg_val[1] << 2) | (reg_val[0] & (~AW8693XS_BIT_CONTRD18_BEMF_PEAK1_L_MASK));
	peak[1] = (reg_val[2] << 2) | (reg_val[0] & (~AW8693XS_BIT_CONTRD18_BEMF_PEAK2_L_MASK));
	peak[2] = (reg_val[3] << 2) | (reg_val[0] & (~AW8693XS_BIT_CONTRD18_BEMF_PEAK3_L_MASK));
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_CONTRD28, reg_val, 3);
	peak[3] = (reg_val[0] << 2) | (reg_val[2] & (~AW8693XS_BIT_CONTRD30_BEMF_PEAK4_L_MASK));
	peak[4] = (reg_val[1] << 2) | (reg_val[2] & (~AW8693XS_BIT_CONTRD30_BEMF_PEAK5_L_MASK));

	for (i = 0; i < 5; i++)
		aw_info("peak[%d] = %d", i, peak[i]);

	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_CONTRD22, reg_val, 6);
	f0_reg = ((reg_val[0] + reg_val[1]) << 4) | (reg_val[4] & 0x0F + (reg_val[4] >> 4) & 0x0F);
	aw_haptic->f0 = AW8693XS_F0_FORMULA(f0_reg);
	aw_info("lra_f0_1 = %d", aw_haptic->f0);
	f0_reg = ((reg_val[1] + reg_val[2]) << 4) | (reg_val[4] & 0x0F + (reg_val[5] >> 4) & 0x0F);
	aw_haptic->f0 = AW8693XS_F0_FORMULA(f0_reg);
	aw_info("lra_f0_2 = %d", aw_haptic->f0);
	f0_reg = ((reg_val[2] + reg_val[3]) << 4) | ((reg_val[5] >> 4) & 0x0F + reg_val[5] & 0x0F);
	aw_haptic->f0 = AW8693XS_F0_FORMULA(f0_reg);
	aw_info("lra_f0_3 = %d", aw_haptic->f0);
}
...
static size_t aw8693xs_get_wav_loop(struct aw_haptic *aw_haptic, char *buf)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_WAVCFG9, reg_val, AW_SEQUENCER_LOOP_SIZE);
...
static uint8_t aw8693xs_get_prctmode(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_PWMCFG3, &reg_val, AW_I2C_BYTE_ONE);
	reg_val >>= 7;
...
static int aw8693xs_read_f0(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_CONTRD22, reg_val, AW_I2C_BYTE_ONE);
	if (reg_val[0] == 0) {
		aw_haptic->f0 = 0;
		aw_err("read_f0 error, 0x85=0");
		return -ERANGE;
	}

#ifdef AW_LRA_F0_DEFAULT
	/* lra_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_CONTRD14, reg_val, AW_I2C_BYTE_TWO);
...
//#else
	/* cont_f0 */
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_CONTRD16, reg_val, AW_I2C_BYTE_TWO);
	f0_reg = (reg_val[0] << 8) | reg_val[1];
	if (!f0_reg) {
		aw_haptic->cont_f0 = 0;
...
static int aw8693xs_get_f0(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG2,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_CONTCFG6,
				 ...
				 (aw_haptic->info.is_enabled_track_en << 7));
	/* enable auto break */
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_PLAYCFG3, &reg_val, AW_I2C_BYTE_ONE);
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG7, cont_config, AW_I2C_BYTE_THREE);
	/* TRACK_MARGIN */
	if (!aw_haptic->info.cont_track_margin) {
		aw_err("aw_haptic->info.cont_track_margin = 0");
	} else {
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG11,
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG2, cont_config, AW_I2C_BYTE_TWO);
...
static int aw8693xs_get_trim_osc_code(struct aw_haptic *aw_haptic)
{
	...
	aw8693xs_reg_unlock(aw_haptic, true);
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_EFCFG9, &osc_trim_s, AW_I2C_BYTE_ONE);
	aw_haptic->osc_cali_data = osc_trim_s;
	aw_haptic->efuse_cali_data = osc_trim_s;
	aw8693xs_reg_unlock(aw_haptic, false);
	if (osc_trim_s > 128)
		ret = osc_trim_s - 256;
	else
...
static uint8_t aw8693xs_rtp_get_fifo_afs(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_SYSST, &reg_val, AW_I2C_BYTE_ONE);
...
static uint8_t aw8693xs_get_osc_status(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_SYSST2, &state, AW_I2C_BYTE_ONE);
...
static void aw8693xs_get_lra_resistance(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_reads(aw_haptic,  AW8693XS_REG_DETCFG1, &reg_val, AW_I2C_BYTE_ONE);
	adc_fs_default = reg_val & (~AW8693XS_BIT_DETCFG1_ADC_FS_MASK);
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_DETCFG2, &reg_val, AW_I2C_BYTE_ONE);
	d2s_gain_default = reg_val & (~AW8693XS_BIT_DETCFG2_D2S_GAIN_MASK);
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG2,
					AW8693XS_BIT_DETCFG2_D2S_GAIN_MASK,
					AW8693XS_BIT_DETCFG2_D2S_GAIN_20);
	d2s_gain = aw8693xs_select_d2s_gain(AW8693XS_BIT_DETCFG2_D2S_GAIN_20);
	if (d2s_gain <= 0) {
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_DETRD1, reg_array, AW_I2C_BYTE_TWO);
...
static void aw8693xs_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	aw8693xs_stop(aw_haptic);
	aw8693xs_ram_init(aw_haptic, true);
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG2, AW8693XS_BIT_DETCFG2_DET_SEQ0_MASK,
				 AW8693XS_BIT_DETCFG2_DET_SEQ0_VBAT);
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG1, AW8693XS_BIT_DETCFG1_DET_GO_MASK,
				 AW8693XS_BIT_DETCFG1_DET_GO_ON);
	usleep_range(3000, 3500);
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG1, AW8693XS_BIT_DETCFG1_DET_GO_MASK,
				 AW8693XS_BIT_DETCFG1_DET_GO_OFF);
...
static ssize_t aw8693xs_get_reg(struct aw_haptic *aw_haptic, ssize_t len, char *buf)
{
	...
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_RSTCFG, reg_array, AW8693XS_REG_RTPDATA);
	haptic_hv_i2c_reads(aw_haptic, (AW8693XS_REG_RTPDATA + 1),
			    &reg_array[AW8693XS_REG_RTPDATA + 1],
			    (AW8693XS_REG_RAMDATA - AW8693XS_REG_RTPDATA - 1));
	haptic_hv_i2c_reads(aw_haptic, (AW8693XS_REG_RAMDATA + 1),
...
static int aw8693xs_offset_os_code(struct aw_haptic *aw_haptic, enum d2s_gain_sel d2s_gain_sel)
{
	...
	if (d2s_gain_sel == AW8693XS_D2S_GAIN) {
		...
		haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSCTRL6,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG1, AW8693XS_BIT_DETCFG1_DET_GO_MASK,
				AW8693XS_BIT_DETCFG1_DET_GO_OFF);
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_DETRD1, reg_val, AW_I2C_BYTE_TWO);
	aw8693xs_ram_init(aw_haptic, false);
...
static int aw8693xs_offset_cali(struct aw_haptic *aw_haptic)
{
	int os_code = 0;

	os_code = aw8693xs_offset_os_code(aw_haptic, AW8693XS_D2S_GAIN);
	aw_info("first os_code = %d", os_code);
	if (os_code > 15 || os_code < -15) {
		aw_info("first offset calibration out of range");
		sw_reset(aw_haptic);
	}

	os_code = aw8693xs_offset_os_code(aw_haptic, AW8693XS_BEMF_D2S_GAIN);
	aw_info("second os_code = %d", os_code);
	if (os_code > 15 || os_code < -15) {
		aw_info("second offset calibration out of range");
		return -ERANGE;
	}
				    ...
				    struct aw_haptic_container *awinic_cont)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_RTPCFG3, reg_array, AW_I2C_BYTE_THREE);

	/* get FIFO_AE and FIFO_AF addr */
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_RTPCFG3, reg_array, AW_I2C_BYTE_THREE);
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_RAMDATA, &awinic_cont->data[i], len);
		i += len;
	}
	...
	while (i < awinic_cont->len) {
		...
		haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_RAMDATA, ram_data, len);
...
static uint64_t aw8693xs_get_theory_time(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val = 0;
	uint32_t fre_val = 0;
	uint64_t theory_time = 0;

	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_SYSCTRL4, &reg_val, AW_I2C_BYTE_ONE);
			     ...
			     struct device_node *np)
{
	...
	if (val != 0)
		aw_info("aw8693xs_trig_config not found");
	else
		memcpy(aw_haptic->info.trig_cfg, trig_config_temp, sizeof(trig_config_temp));

	val = of_property_read_u32(np, "aw8693xs_bst_vol_default",
				   &aw_haptic->info.bst_vol_default);
	if (val != 0)
		aw_info("aw8693xs_bst_vol_default not found");
	...
	aw_info("lower power = %d", aw_haptic->info.is_enabled_low_power);

	aw_haptic->info.is_enabled_vbat_pro = of_property_read_bool(np,
						"aw8693xs_is_enabled_vbat_pro");
	aw_info("vbat pro = %d", aw_haptic->info.is_enabled_vbat_pro);
...
static void aw8693xs_set_vbat_pro_params(struct aw_haptic *aw_haptic)
{
	...
	for (i = 0; i < 2; i++) {
		...
		aw_info("set bst_max = %u mV, reg_val[0] = 0x%02x", bst_max, reg_val[0]);
		aw_info("set ipeak = %u mA, reg_val[1] = 0x%02x", ipeak, reg_val[1]);

		if (i == 0) {
			reg_val[0] = (reg_val[0] << 4) | reg_val[1];
			haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_SYSCTRL8, &reg_val[0], AW_I2C_BYTE_ONE);
		} else if (i == 1) {
			reg_val[0] = (reg_val[0] << 4) | reg_val[1];
			haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_SYSCTRL9, &reg_val[0], AW_I2C_BYTE_ONE);
		}
	}
	...
	case AW8693XS_PRO3_GAIN_1:
		...
		aw_info("set vbat pro3_gain = 1");
		break;
	case AW8693XS_PRO3_GAIN_3_4:
		reg_val[0] = AW8693XS_BIT_SYSCTRL7_PRO3_GAIN_0P75;
		aw_info("set vbat pro3_gain = 0.75");
		break;
	case AW8693XS_PRO3_GAIN_1_2:
		reg_val[0] = AW8693XS_BIT_SYSCTRL7_PRO3_GAIN_0P5;
		aw_info("set vbat pro3_gain = 0.5");
		break;
	case AW8693XS_PRO3_GAIN_0:
		reg_val[0] = AW8693XS_BIT_SYSCTRL7_PRO3_GAIN_0;
		aw_info("set vbat pro3_gain = 0");
		break;
	default:
		reg_val[0] = AW8693XS_BIT_SYSCTRL7_PRO3_GAIN_0P5;
		aw_err("vbat pro3 gain error, default pro3 gain = 1/2");
		break;
	}
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSCTRL7,
...
static void aw8693xs_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_SYSCTRL5, reg_val, AW_I2C_BYTE_ONE);
	/* Cont drv2 lvl */
	...
	/* Get gain */
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_PLAYCFG2, reg_val, AW_I2C_BYTE_ONE);
	aw_haptic->gain = reg_val[0];
	/* Get wave_seq */
	haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	aw_haptic->index = reg_val[0];
	memcpy(aw_haptic->seq, reg_val, AW_SEQUENCER_SIZE);
	/* set d2s gain and bemf d2s gain */
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_DETCFG2,
				AW8693XS_BIT_DETCFG2_D2S_GAIN_MASK, aw_haptic->info.d2s_gain);
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSCTRL6,
			AW8693XS_BIT_SYSCTRL6_BEMF_D2S_GAIN_MASK,
			aw_haptic->info.bemf_d2s_gain);
	/* Set gain_bypass */
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSCTRL4,
				 AW8693XS_BIT_SYSCTRL4_GAIN_BYPASS_MASK,
				 aw_haptic->info.gain_bypass);

	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_CONTCFG5,
				 AW8693XS_BIT_CONTCFG5_BRK_GAIN_MASK, aw_haptic->info.cont_brk_gain);
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG10,
	...
	haptic_hv_i2c_write_bits(aw_haptic, AW8693XS_REG_SYSCTRL12,
	...
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_SYSCTRL7, &reg_val[0], AW_I2C_BYTE_ONE);
	aw8693xs_reg_unlock(aw_haptic, true);
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_ANACFG17, &reg_val[1], AW_I2C_BYTE_ONE);
	haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_ANACFG20, &reg_val[2], AW_I2C_BYTE_ONE);
	aw8693xs_reg_unlock(aw_haptic, false);
	/* set vbat pro params */
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		mutex_lock(&aw_haptic->lock);
		haptic_hv_i2c_reads(aw_haptic, AW8693XS_REG_CONTCFG6, reg_array, AW_I2C_BYTE_ONE);
		...
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG6, reg_array, AW_I2C_BYTE_TWO);
		mutex_unlock(&aw_haptic->lock);
	}
				   ...
				   const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		mutex_lock(&aw_haptic->lock);
		haptic_hv_i2c_writes(aw_haptic, AW8693XS_REG_CONTCFG8, reg_array, AW_I2C_BYTE_TWO);
		mutex_unlock(&aw_haptic->lock);
	}
...
static int aw8693xs_creat_node(struct aw_haptic *aw_haptic)
{
	int ret = 0;
	...
	if (ret < 0) {
		aw_err("error create aw8693xs sysfs attr files");
		return ret;
	}
	return 0;
}
...
struct aw_haptic_func aw8693xs_func_list = {
	...
	.judge_rtp_going = aw8693xs_judge_rtp_going,
