...
static void aw8671x_set_bst_vol(struct aw_haptic *aw_haptic, uint8_t bst_vol)
{
	if (bst_vol & 0xf0) /* 4 bit */
		bst_vol = 0x0F;
	i2c_w_bits(aw_haptic, AW8671X_REG_PLAYCFG1,
		   AW8671X_BIT_PLAYCFG1_CP_CODE_MASK, bst_vol);
}
...
static uint8_t aw8671x_get_glb_state(struct aw_haptic *aw_haptic)
{
	...
	i2c_r_bytes(aw_haptic, AW8671X_REG_GLBRD5, &glb_state, AW_I2C_BYTE_ONE);
	return glb_state;
}
...
static void aw8671x_wait_enter_standby(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val = 0;
	uint32_t cnt = 100;
	...
	while (cnt--) {
		...
		aw_dbg("wait for standby, glb_state = 0x%02x!", reg_val);
...
static int aw8671x_juge_rtp_going(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->rtp_routine_on
		|| (glb_state == AW8671X_BIT_GLBRD5_STATE_RTP_GO)) {
...
static ssize_t aw8671x_get_ram_data(struct aw_haptic *aw_haptic, char *buf)
{
	...
	while (i < aw_haptic->ram.len) {
		...
		i2c_r_bytes(aw_haptic, AW8671X_REG_RAMDATA, ram_data, size);
		for (j = 0; j < size; j++) {
...
static void aw8671x_read_f0(struct aw_haptic *aw_haptic)
{
	uint8_t reg_val[4] = {0};
	uint32_t f0_reg = 0;
	uint64_t f0_tmp = 0;
	...
	/* lra_f0 */
	i2c_r_bytes(aw_haptic, AW8671X_REG_CONTCFG14, reg_val, 4);
...
static void aw8671x_get_vbat(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->vbat > AW8671X_VBAT_MAX) {
		aw_haptic->vbat = AW8671X_VBAT_MAX;
			    ...
			    struct device_node *np)
{
	...
	val = of_property_read_u8(np, "aw8671x_max_bst_vol",
				  &aw_haptic->info.max_bst_vol);
	if (val != 0)
		aw_info("aw8671x_max_bst_vol not found");
	...
	val = of_property_read_u8(np, "aw8671x_bst_vol_default",
				  &aw_haptic->info.bst_vol_default);
	if (val != 0)
		aw_info("aw8671x_bst_vol_default not found");
	val = of_property_read_u8(np, "aw8671x_bst_vol_ram",
				  &aw_haptic->info.bst_vol_ram);
	if (val != 0)
		aw_info("aw8671x_bst_vol_ram not found");
	val = of_property_read_u8(np, "aw8671x_bst_vol_rtp",
				  &aw_haptic->info.bst_vol_rtp);
	if (val != 0)
		aw_info("aw8671x_bst_vol_rtp not found");
	...
	aw_dbg("aw_haptic->info.bst_vol_default: %d",
		aw_haptic->info.bst_vol_default);
	aw_dbg("aw_haptic->info.bst_vol_ram: %d",
		aw_haptic->info.bst_vol_ram);
	aw_dbg("aw_haptic->info.bst_vol_rtp: %d",
		aw_haptic->info.bst_vol_rtp);
}
...
static void aw8671x_misc_para_init(struct aw_haptic *aw_haptic)
{
	...
	reg_val[0] = AW_I2C_ADDR;
	i2c_w_bytes(aw_haptic, AW8671X_REG_SYSCTRL4, &reg_val[0],
		    AW_I2C_BYTE_ONE);
	i2c_r_bytes(aw_haptic, AW8671X_REG_PLAYCFG1, reg_val, AW_I2C_BYTE_TWO);
	aw_haptic->gain = reg_val[1];
	if (aw_haptic->info.bst_vol_default > 0)
		aw_haptic->vmax = aw_haptic->info.bst_vol_default;
	else
		aw_haptic->vmax = reg_val[0] & AW8671X_BIT_PLAYCFG1_CP_CODE;
	i2c_r_bytes(aw_haptic, AW8671X_REG_WAVCFG1, reg_val, AW_I2C_BYTE_EIGHT);
	aw_haptic->index = reg_val[0] & AW8671X_BIT_WAVCFG_WAVSEQ_NUMBER;
...
static ssize_t aw8671x_bst_vol_show(struct device *dev,
...
static ssize_t aw8671x_bst_vol_store(struct device *dev,
...
static DEVICE_ATTR(bst_vol, S_IWUSR | S_IRUGO, aw8671x_bst_vol_show,
		   aw8671x_bst_vol_store);
