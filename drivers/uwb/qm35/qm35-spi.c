			 ...
			 loff_t *off)
{
	...
	if (copy_from_user(p->data, buf, (u16)len)) {
		ret = -EFAULT;
		goto free;
	}
...
static int hsspi_irqs_setup(struct qm35_ctx *qm35_ctx)
{
	...
	if (wake_on_ssirq) {
		...
		if (ret) {
			dev_err(&qm35_ctx->spi->dev,
				"%s: enable_irq_wake returned %d", __func__,
				ret);
			return ret;
		}
	}
...
static int qm35_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(uwb_clk)) {
		dev_err(&qm35_ctx->spi->dev, "%s: uwb_clk not found", __func__);
	} else {
		ret = clk_prepare_enable(uwb_clk);
		if(ret)
			dev_err(&qm35_ctx->spi->dev, "%s: uwb_clk enable failed", __func__);
	}

	qm35_ctx->gpio_csn = devm_gpiod_get_optional(&spi->dev, "csn",
          GPIOD_OUT_LOW);

	if (!qm35_ctx->gpio_csn)
	{
		dev_err(&qm35_ctx->spi->dev, "%s: csn-gpios not found", __func__);
	} else {
		dev_err(&qm35_ctx->spi->dev, "%s: csn-gpios found!!", __func__);
	}
	global_temporary_gpio_csn = qm35_ctx->gpio_csn;
