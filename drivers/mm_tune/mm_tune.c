...
#define FREE_THRESHOLD  (500*1024*1024)
...
static void cma_enable_workfn(struct work_struct *work)
{
	...
	if (!cma_bypass)
		return;
	// pr_info("tune enable cma %d\n", cma_bypass);
	/* Get system memory information */
	si_meminfo(&si);
	if (((si.freeram * si.mem_unit ) < mem_thresh ) || (!bypass_cycle))
	{
		pr_info("tune free %ld KB cnt %d \n", si.freeram << 2 , bypass_cycle);
		cma_bypass = 0;
	}else
	{
		pr_info("tune next cma cnt %d\n",  bypass_cycle);
		bypass_cycle--;
