...
static int st54spi_parse_dt(struct device *dev, struct st54spi_data *pdata)
{
	...
	if (pdata->power_or_nreset_gpio_mode == POWER_MODE_ST54H) {
		...
		pdata->pctrl_mode_spi = pinctrl_lookup_state(
			pdata->pctrl, "pinctrl_state_mode_spi");
		if (IS_ERR(pdata->pctrl_mode_spi)) {
			dev_err(dev,
				"%s: Unable to find pinctrl_state_mode_spi: %d\n",
				__FILE__, PTR_ERR(pdata->pctrl_mode_spi));
			return -EINVAL;
		}

		pdata->pctrl_mode_idle = pinctrl_lookup_state(
			pdata->pctrl, "pinctrl_state_mode_idle");
...
static int st54spi_probe(struct spi_device *spi)
{
	...
	int status, ret;
	unsigned long minor;
#if 0 // MTK original code
	struct mtk_chip_config *chip_config = (struct mtk_chip_config *)spi->controller_data;
#else
...
#else
	chip_config = devm_kzalloc(&spi->dev, sizeof(chip_config), GFP_KERNEL);
	if (chip_config == NULL) {
		dev_err(&spi->dev, "%s: alloc chip config failed\n", __func__);
		status = -ENOMEM;
	} else {
		chip_config->cs_setuptime = st54spi_chip_info.cs_setuptime;
		chip_config->cs_idletime = st54spi_chip_info.cs_idletime;
		...
		dev_info(&spi->dev, "Added into chip_info!\n");
	}
#endif
	if (status == 0)
