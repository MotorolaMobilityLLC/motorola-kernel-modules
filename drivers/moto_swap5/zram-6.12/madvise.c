		...
		struct list_head *list, unsigned long start, unsigned long end)
{
	...
	for (;;) {
		...
		/* Still start < end. */
		//do not return -ENOMEM,continue to walk the rest
		if (!vma){
			//return -ENOMEM;
			break;
		}
		...
		error = zram_madvise(zram, vma, list, start, tmp);
		prev = vma;
		if (error)
			return error;
		start = tmp;
		...
		struct list_head *list, unsigned long start, size_t len_in)
{
	...
	pr_debug("[zram-ctx]  do madvise start %lx, len%lx", start, len_in);
		...
		unsigned int flags, ssize_t *ret, bool *bypass)
{
	...
	pr_debug("[zram-ctx]  process command %d iovec len %lx\n", arg, vlen);
...
out:
	...
	pr_debug("[zram-ctx]  exit with result %lx\n", err);
	*bypass = true;
